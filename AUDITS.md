...
| System Design DEPTH (anti-drift) | `config/auditSysdDepth.mjs` (`npm run audit:sysd-depth`; gate a set with `--require id1,id2`) | enforces **SD_DEPTH_STANDARD.md** — gates on DEPTH, not topic count (the track hit count parity 179>177 while each topic stayed ~500 words / 0 diagrams; a user spotted the gap). Classifies every case-study topic DEEP / PARTIAL / SHALLOW: DEEP needs sections requirements+estimation+hld+api+data_model+bottlenecks + ≥2 deep_dive (via block `section` tags) + a well-formed authored `visual_aid.svg` + ≥1200 words + ≥3 exercises + ≥3 gaps. PARTIAL (started but incomplete) = HARD FAIL. Prints DEEP/total coverage. Coverage: **29/143 DEEP** (b1: URL shortener, rate limiter, news feed, chat, KV store; b2: Instagram, video streaming, ride-sharing, web crawler, distributed cache, typeahead; b3: search engine, message queue, leaderboard, notifications, payment, maps; b4: file sync, unique-id, object store/S3, proximity, distributed lock, job scheduler; b5: distributed counter, top-k/trending, CDN, log aggregation, metrics/monitoring, saga orchestrator), each render-verified (svg visible) + grading-clean. Seeds: `seed:sysd-depth-pilot`..`-5`. Raise coverage by deepening more case studies (114 remain). |
