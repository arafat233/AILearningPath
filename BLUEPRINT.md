...
- **System Design track (GAP #2 MVP, 2026-06-04):** a NEW standalone professional track `pro_sysd` ("System Design — Scalability & Architecture", slug `system-design`), separate from `pro_java`/`pro_lld`. One module `sysd_m1` (System Design Fundamentals), **12 topics · 38 exercises** (all `pattern_match` — the natural modality for "which approach/component/trade-off?"): the interview framework, back-of-envelope estimation, vertical-vs-horizontal scaling, load balancing, caching, SQL-vs-NoSQL, replication & sharding (incl. consistent hashing), CAP & consistency, message queues, + 3 case studies (URL shortener, distributed rate limiter, news feed). Architecture topics carry a `teaching.visual_aid` whose `description` is an `"A → B → C"` flow that the existing `components/pro/VisualAid.jsx` auto-renders as a boxes-and-arrows diagram (no authored SVG). Pure content build — the data-driven Pro stack means no frontend change. Seeded by `config/seedSysdTrack.js` (`npm run seed:sysd`), totals via the new generic `config/proTrackTotals.mjs`; audited by `config/auditSysdTrack.mjs` (`npm run audit:sysd`, 38/38) + live round-trip `scripts/acceptanceSysd.mjs` (PASS). **Backend change:** `validators/proValidator.js` `trackSlugPattern` now allows hyphens (`/^[a-z][a-z0-9_-]*$/`) so hyphenated slugs like `system-design` pass the `getTrack`/`getModule` param routes (existing `java`/`lld` slugs still match — verified by both acceptance scripts).
