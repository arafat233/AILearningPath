...
- **Pro Java — Phase 1/2 capabilities (verified, 2026-06-02):**
  ...
  - **Recursion-Tree visualizer** (Phase I1): `recursion-tree` kind — N-ary call tree with memo-hit/pruned states; 3 demos (naive fib, memoised fib, subsets backtracking). Wired to M40-T1 (subsets) + M41-T1 (fib_memo). `RecursionTreeVisualizer.jsx` + `recursionTree.js`. Browser-verified.
...
- **Pro Java — parity modules + pages (verified, 2026-06-02):** Bitwise M47 + `BitwiseVisualizer` (D3.1), Recursion Patterns M48 (D3.2), Pattern Atlas `/pro/java/patterns` (D3.4), free-tier 10 lighthouse topics + `/pro/preview/:topicId` public view (D5.1), Modern Java M49 / Engineering Hygiene M50 / Technical Communication M51 (J1–J3). All round-trip-verified per `PRO_EXERCISE_TYPE_CHECKLIST.md`.
...
- **Track-2 step-by-step animation player (2026-06-03):** `ProExercise.animation` (Mixed) + `components/pro/StepPlayer.jsx` (shared shell + per-`kind` stages) in `ProExerciseRunner`. 5 renderers — **array-pointers**, **stack**, **grid**, **linked-list**, **tree** (heap reuses tree, DP reuses grid). **100% coverage: all 598 animatable DSA problems** animated (`config/seedAnimationCoverage1..24.js`), audited by `config/auditAnimations.mjs` (0 broken). Play/pause/step/scrub.
- **Notes / Highlights / Notebook (GAP #3, 2026-06-04):** app-wide (Pro + K-12) notes & anchored highlights. New `Note` model (`models/noteModels.js`) — one doc per note/highlight, denormalized source context (scope/kind/refId/trackKey/subject/title/url) so the Notebook needs no joins. Backend: `services/noteService.js` + `controllers/noteController.js` + `routes/noteRoutes.js` at `/api/v1/notes` (GET `/` notebook search+filter, GET `/for/:kind/:refId`, GET `/tags`, GET `/stats`, POST `/`, PATCH `/:id`, DELETE `/:id`; Joi-validated, ownership-scoped). Frontend: `components/NotesPanel.jsx` (per-item composer + list; select-text-to-highlight via the CSS Custom Highlight API — no DOM mutation; paints `::highlight(stellar-note)`), `pages/Notebook.jsx` (`/notebook`, searchable + scope/type/tag filters), nav entry in both K-12 & Pro sidebars. Wired into `ProExerciseRunner` (Pro exercises) and `NcertTopicView` (K-12 lessons). Audited by `config/auditNotes.mjs` + `note.service` Jest + `Notes.test.jsx`. (Practice-question surface = fast-follow.)
- **Chat with your own uploads (Open-Notebook U, 2026-08-25):** students upload coaching notes / PDFs / pasted text and the AI tutor answers from them alongside NCERT. New models in `models/userChunkModels.js` — `UserSource` (one uploaded document: name/mime/bytes/pages/chunkCount/subject/status) and `UserChunk` (the per-user mirror of `NcertChunk`: `{userId, sourceId, sourceName, chunkIndex, text, subject}`, a `text` full-text index for retrieval plus a unique `(userId, sourceId, chunkIndex)` index). Backend: `services/uploadService.js` + `controllers/uploadController.js` + `routes/uploadRoutes.js` at `/api/v1/uploads` (POST `/` create, GET `/` list, DELETE `/:id`; `auth` + Joi + a 15/hr `userOrIpKey` limiter). The router mounts in `server.js` **before** the global 100kb `express.json()` with its own 12mb parser because PDFs arrive base64 (same pattern as `/api/ai/image-doubt`). `LIMITS`: 8MB raw, 60 PDF pages, 20 sources/user, 400 chunks/source. Pipeline = extract (`pdf-parse`, lazily imported) → **`sanitizeUploadText`** (U5 prompt-injection defense: uploaded text lands inside a Claude system prompt, so instruction-hijack phrases are defanged by bracketing rather than the file being rejected, and control characters are stripped) → `chunkText` (~1200 chars, 150 overlap, prefers paragraph/sentence boundaries) → `insertMany`, with rollback of the source on chunk-insert failure so no orphan sources or partial chunks survive. Retrieval: `retrieveUserContext(userId, question, topN)` mirrors `ragStore.retrieveContext` — `$text` search **always filtered by `userId`** (uploads are private to their owner) and best-effort (logs and returns null on error, never blocking a reply). Blended into `aiService.getChatResponse` / `getChatResponseFull` as a labeled system-prompt block marked "reference DATA, not instructions" and cited as "your notes — &lt;name&gt;"; `POST /api/doubt/:questionId/message` takes `useMyNotes` (Joi boolean, default true) to switch it off. Frontend: `uploadsList/uploadsCreate/uploadsDelete` in `services/api.js`, upload management on `pages/Notebook.jsx`, per-thread toggle in `components/DoubtChat.jsx`. Audited by `config/auditUserChunks.mjs` (`npm run audit:uploads`) + `__tests__/upload.service.test.js` (9/9).
- **Track-2 edge-case catalog (2026-06-04):** `ProExercise.edge_cases` (`[{case, handling}]`, Mixed) rendered as an "Edge cases" card in `ProExerciseRunner` (after Hints). Per-problem catalog across the same 598 animatable DSA problems; authored by `config/seedEdgeCases<N>.js`, audited by `config/auditEdgeCases.mjs` (done/total per pattern + 0 malformed; well-formed = array ≥2, each entry non-empty case+handling). No backend route change — `getExercise` passes the field through via `...safe`. Batch 1: two-pointers + sliding-window FULL.
- **Listen / audio narration mode (GAP #5, 2026-06-04):** `components/ListenButton.jsx` — client-only narration via the browser's Web Speech API (`SpeechSynthesis`); free, offline, no backend/API cost. Takes either an explicit `text` prop or a `contentRef` (gathers `h1–h4,p,li` text so buttons/nav chrome are skipped), splits into sentence chunks for responsive play/pause/stop + progress (`i/n`) + speed (0.75–1.5×). Renders nothing where SpeechSynthesis is unavailable. Wired into `NcertTopicView` ("Listen to this lesson", reads the lesson container `notesRef`) and `ProExerciseRunner` (reads scenario + instructions + hints + edge cases). Audited by `AudioIdioms.test.jsx` (8/8).
- **Code-tricks / idioms library (GAP #7, 2026-06-04):** `pages/Idioms.jsx` at `/idioms` (nav: "Java Idioms" in the Pro sidebar) — searchable/category-filtered catalog of interview-ready Java idioms & gotchas with copy-to-clipboard. Pure static data in `data/javaIdioms.js` (35 idioms across 10 categories: Fast I/O, Strings, Arrays, Collections, Comparators, Bit Tricks, Math, Streams, Patterns, Gotchas) — add an entry and it appears immediately. No backend. Audited by `scripts/auditIdioms.mjs` (35/35, 0 malformed, 0 dup ids) + render-tested in `AudioIdioms.test.jsx`.
- **LLD depth pass — 129/129 case studies DEEP (2026-08-25):** count parity was reached long ago; DEPTH was not gated until now. Every one of the 129 `pro_lld` case-study topics has been rewritten to `LLD_DEPTH_STANDARD.md`: `teaching.blocks` tagged `requirements` / `entities` / `design` / `code` (`kind:"code"`, real Java) / **3×** `deep_dive` / `tradeoffs`, an authored UML-style `visual_aid.svg` (boxes + labelled relations + a mechanism note), ~2,000–4,140 words of prose per topic, ≥3 `commonGaps`, and 3 new `pattern_match` exercises per topic at positions 11–13 (`<topicId>_pm_d1..d3`). Authored in 21 batches (`config/seedLldDepthBatch1..21.js`, `npm run seed:lld-depth-N`), each gated with `node config/auditLldDepth.mjs --require <ids>` before the next was launched. Track now **22 modules · 172 topics · 907 exercises · 27,665 XP**. Audits: `npm run audit:lld-depth` → 129/129 DEEP, 0 PARTIAL, 0 SHALLOW; `npm run audit:lld` → 0 malformed / 0 orphans / 0 dup ids / 152 visual_aids; Jest 54 suites / 572 tests PASS. `auditLldDepth.mjs` also gained `--list-shallow`, which prints the remaining roster with exact `topicId | topicNumber | slug | name` so batch assignments come from audit output rather than hand-counting.
- **Low-Level Design track (GAP #4, 2026-06-04):** a NEW standalone professional track `pro_lld` ("Low-Level Design — OOD, SOLID & Patterns", slug `lld`), separate from `pro_java`. **5 modules · 36 topics · 112 exercises** (79 pattern_match, 23 predict_output, 10 code_scratch), 3450 XP:
  - `lld_m1` Object-Oriented Design Foundations (12 topics): OOP pillars, SOLID, UML, Strategy/Observer/Factory/Singleton/Decorator/Adapter, Parking-Lot & LRU-Cache case studies, LLD-interview framework.
  - `lld_m2` Creational Patterns (3): Builder, Abstract Factory, Prototype.
  - `lld_m3` Structural Patterns (5): Facade, Composite, Proxy, Bridge, Flyweight.
  - `lld_m4` Behavioral Patterns (8): Command, State, Template Method, Chain of Responsibility, Iterator, Mediator, Memento, Visitor.
  - `lld_m5` LLD Case Studies (8): Elevator, Vending Machine, Splitwise, Rate Limiter, Tic-Tac-Toe, Snake & Ladder, Notification Service, Logging Framework.

  **UML class diagrams (polish, 2026-06-04):** all 22 GoF-pattern topics carry a `teaching.visual_aid.svg` UML class diagram, generated by `config/umlSvg.mjs` (a theme-adaptive class-diagram generator — `currentColor` + neutral fills, proper UML markers: hollow-triangle inherit/implement, diamond compose/aggregate, open-arrow assoc/depend) and attached via `config/seedLldDiagrams.js` (`$set teaching.visual_aid`, preserving `teaching.blocks`; `npm run seed:lld-diagrams`). Rendered by the existing `components/pro/VisualAid.jsx` authored-SVG tier — still no frontend change. `scripts/previewLldDiagrams.mjs` emits a light+dark preview HTML for visual QA.

  Pure content build: the entire Pro stack (ProTrackPicker, `getTrack`, `enroll`, runner, progress, certificates) is data-driven, so the live track appears end-to-end with **no backend/frontend code change**. Seeds: `config/seedLldTrack.js` (+ `seedLldCreational/Structural/Behavioral/CaseStudies.js`), idempotent upsert-by-id; `npm run seed:lld-all`. Track totals are recomputed from the DB via `config/lldTotals.mjs` so any seed keeps them correct. **Three audit gates:** `npm run audit:lld` (structural 112/112, 0 malformed/orphan/dupe), `npm run acceptance:lld` (live HTTP — all modules reachable, every type 200, grading correct), `npm run verify:lld-code` (all 10 code_scratch reference solutions compile + match expected_stdout on real Judge0). Java stays JDK-13-safe; test-case types follow the proven convention — pattern_match→`pattern_match`, predict_output→`text_match`, code_scratch→`execution`+`must_compile`. NOTE: exerciseId validator only allows `_(ex|pm)_` suffixes, and `fill_blank` has **no grader case** (default→fail), so it was avoided.
- **System Design track (GAP #2, 2026-06-04):** a NEW standalone professional track `pro_sysd` ("System Design — Scalability & Architecture", slug `system-design`), separate from `pro_java`/`pro_lld`. **4 modules · 34 topics · 104 exercises** (all `pattern_match` — the natural modality for "which approach/component/trade-off?"), 3530 XP:
  - `sysd_m1` Fundamentals (12): interview framework, estimation, scaling, load balancing, caching, SQL-vs-NoSQL, replication & sharding (+consistent hashing), CAP & consistency, message queues, + URL shortener / rate limiter / news feed.
  - `sysd_m2` Building Blocks & Distributed Patterns (10): API design (REST/gRPC/GraphQL), API gateway/BFF, CDN, microservices vs monolith, service discovery, idempotency & retries, resilience (circuit breaker/bulkhead/timeout), consensus & leader election, distributed transactions (2PC/Saga), bloom filters.
  - `sysd_m3` Case Studies — Social & Communication (6): chat (WhatsApp), search autocomplete, notifications at scale, Instagram, live presence, pastebin.
  - `sysd_m4` Case Studies — Infrastructure & Scale (6): video streaming, ride-sharing (Uber), file sync (Dropbox), web crawler, distributed KV store (Dynamo), ticket booking (Ticketmaster).

  Architecture topics (21 of them) carry a `teaching.visual_aid` whose `description` is an `"A → B → C"` flow that the existing `components/pro/VisualAid.jsx` auto-renders as a boxes-and-arrows diagram (no authored SVG). Pure content build — the data-driven Pro stack means no frontend change. Seeds: `config/seedSysdTrack.js` + `seedSysd{BuildingBlocks,CasesSocial,CasesInfra}.js` (`npm run seed:sysd-all`); totals via the generic `config/proTrackTotals.mjs`. Audited by `config/auditSysdTrack.mjs` (`npm run audit:sysd`, 104/104) + live round-trip `scripts/acceptanceSysd.mjs` (PASS, all 4 modules). **Backend change:** `validators/proValidator.js` `trackSlugPattern` now allows hyphens (`/^[a-z][a-z0-9_-]*$/`) so hyphenated slugs like `system-design` pass the `getTrack`/`getModule` param routes (existing `java`/`lld` slugs still match — verified by both acceptance scripts).
- **Community / Newsletter engine (GAP #8, 2026-06-04):** a global, user-generated content + community surface at `/community` (in both K-12 and Pro sidebars — it's cross-track). One unified `CommunityPost` model (`models/communityModels.js`) with `kind: "article" | "question"` — articles are the newsletter/blog surface, questions are a global Q&A feed (complementing the existing per-topic `ProTopicDiscussion`). Both support embedded comments, upvotes (denormalized `upvoteCount`/`commentCount` for cheap feed sorting), reporting, and **admin moderation** (remove/restore/pin/clear-reports/delete). Backend vertical slice: `services/communityService.js` + `controllers/communityController.js` + `routes/communityRoutes.js` mounted at **`/api/v1/community`** (feed list w/ kind+tag+search+sort, get, create, edit, delete, upvote, comment, delete-comment, report; admin routes under `/community/admin/*` gated by `adminAuth`). Joi-validated, thin controllers, `AppError`s. Frontend: `pages/Community.jsx` (feed: kind tabs, New/Top sort, search, inline composer) + `pages/CommunityPost.jsx` (detail: body, upvote, comments, report/delete) + api helpers + routes `/community` & `/community/:id`. Open to any authed user (NOT enrollment-gated). Audited by `config/auditCommunity.mjs` (conformance, 0 malformed) + `scripts/acceptanceCommunity.mjs` (full lifecycle) + `community.service` Jest (17) + `Community.test.jsx` (6). Starter posts seeded by `config/seedCommunitySeedPosts.js` (`npm run seed:community`, authored by a "Stellar Team" system account).
- **LLD/SD content depth + lesson renderer (2026-06-04):** benchmarked vs AlgoMaster (LLD 173 chapters / SD 177) and closed the gap on rendering AND depth. (1) **Renderer:** `ProTopicView` previously dumped `teaching.blocks` into a generic "More" section (code as plain text); added a `TeachingBlocks` component (concept→card, code→syntax-highlighted `CodeBlock`) + a "Lesson" nav entry + `blocks` in `teachingKnown`. Benefits LLD, SD, and the M48 recursion module. Test: `ProTopicViewLesson.test.jsx`. (2) **Depth:** all 70 LLD+SD topics deep-enriched to ~7–9 blocks each — code examples, real-world JDK/Spring/AWS/Netflix usage, pros/cons, and "common interview follow-ups". Counts unchanged (LLD 36/112, SD 34/104); audits still PASS. GOTCHA: re-seeding a module overwrites `teaching`, dropping `visual_aid`, so the `seed:*-all` chains run the diagrams seed LAST.
- **LLD + System Design breadth expansion (2026-06-04):** closed AlgoMaster section-coverage gaps by adding 5 modules. **System Design** → 7 modules / 51 topics (added M5 Networking & Protocols, M6 Storage/Big-Data/Scale-Structures, M7 Deployment/Observability/Security). **LLD** → 7 modules / 52 topics (added M6 Design Principles & Class Relationships — DRY/KISS/YAGNI/Law-of-Demeter/SoC/coupling-cohesion/composition-over-inheritance/UML-relationships; M7 Enterprise & Concurrency Patterns — Repository/MVC/DI/Producer-Consumer/Thread-Pool/Specification/Null-Object/Game-Loop). Seeds: `seedSysd{Networking,Storage,Ops}.js`, `seedLld{Principles,MorePatterns}.js`; folded into `seed:sysd-all`/`seed:lld-all` (diagrams seed still LAST in the LLD chain). All pattern_match, audits PASS. Also added **LLD M8 More Case Studies** (ATM, Chess, Stack Overflow, Movie Booking, Shopping Cart, Car Rental, Meeting Scheduler, In-Memory File System) → LLD 8 modules / 60 topics / 184 ex; and expanded the **Pattern Atlas** (`pages/professional/ProPatternAtlas.jsx` PATTERN_CATALOG) from 15 → **72 coding patterns** (each id/label/icon/when), closing the AlgoMaster "70+ patterns" gap. Final tally: System Design 51 topics, LLD 60 topics, 72-pattern atlas.
- **LLD + System Design case-study expansion — closing AlgoMaster's raw count (2026-06-05, local; not yet pushed/deployed):** added **4 new case-study modules per track** (8 topics + 24 pattern_match exercises each, idempotent, totals recomputed). **LLD → 12 modules / 92 topics / 280 exercises / 9265 XP:** M9 `seedLldCases3.js` (Minesweeper, Traffic Signal, Coffee Vending, Task/Inventory/Library/Restaurant Mgmt, Pub-Sub), M10 `seedLldCases4.js` (Hotel Booking, Food Delivery, Digital Wallet, Deck of Cards, Auction, Calendar, Text Editor undo/redo, Concurrent Cache), M11 `seedLldCases5.js` (Music Player, Ride-Sharing OOD, Cricket Scoreboard, Snake Game, LFU Cache, Bowling, Trie Autocomplete, Download Manager), M12 `seedLldCases6.js` (Hospital Mgmt, Airline Reservation, Spreadsheet, Whiteboard, Expression Evaluator/Interpreter, Object/Connection Pool, Coupon Engine, Course Registration). **System Design → 11 modules / 83 topics / 251 exercises / 8825 XP:** M8 `seedSysdCases3.js` (Distributed Cache, Message Queue, Leaderboard, Ad-Click Aggregator, Job Scheduler, Unique-ID, Object Store/S3, Proximity), M9 `seedSysdCases4.js` (Google Docs collab-edit, Payment System, Google Maps, Distributed Lock, Recommendation, Matching Engine, Email/Gmail, Flash Sale), M10 `seedSysdCases5.js` (Twitter fanout, Distributed Counter, Slack, Code Judge, Live Streaming, Webhook Delivery, CI/CD, Fraud Detection), M11 `seedSysdCases6.js` (Airbnb/Booking, Search Engine, Comment System, Metrics/Monitoring, A/B Testing, Top-K/Trending, Distributed Tracing, Tinder). All folded into `seed:lld-all` (diagrams seed still LAST) and `seed:sysd-all`. Each topic = 4 `teaching.blocks` + `interviewRelevance` + `commonGaps`; most carry a flow `teaching.visual_aid` (LLD 39 visual_aids, SD 52). `npm run audit:lld` = 92/92 PASS, `npm run audit:sysd` = 83/83 PASS. Pure data build, no code change. Remaining to match AlgoMaster (LLD 173 / SD 177): ~81 LLD + ~94 SD — genuine distinct topics still available (concurrency primitives, board games, parsers, frameworks; distributed config, conferencing, IoT ingest, multiplayer servers).
- **LLD M13 + SD M12 (2026-06-05, local; not yet pushed/deployed):** another batch per track (8 topics + 24 pattern_match each). **LLD → 13 modules / 100 topics / 304 exercises / 10160 XP:** M13 `seedLldCases7.js` — concurrency primitives (Bounded Blocking Queue, Read-Write Lock), games (Connect Four, Sudoku validator/solver), a JSON parser, a DI container, a plugin system, a configuration manager. **System Design → 12 modules / 91 topics / 275 exercises / 9715 XP:** M12 `seedSysdCases7.js` — feature-flag/config service, video conferencing (Zoom/SFU), IoT platform (MQTT), multiplayer game server (authoritative), stock brokerage (Robinhood), content moderation, live location sharing, price-comparison aggregator. Wired into `seed:lld-all` (diagrams LAST) / `seed:sysd-all`. `audit:lld` 100/100 PASS, `audit:sysd` 91/91 PASS. Remaining to match AlgoMaster: ~73 LLD + ~86 SD (genuine topics still available).
- **LLD M14 + SD M13 (2026-06-05, local; not yet pushed/deployed):** another batch per track. **LLD → 14 modules / 108 topics / 328 exercises / 11025 XP:** M14 `seedLldCases8.js` — URL Shortener (OOD), Circuit Breaker, Tetris, Poker Hand Evaluator, Conway's Game of Life, CLI Argument Parser, Validation Framework, Battleship. **System Design → 13 modules / 99 topics / 299 exercises / 10615 XP:** M13 `seedSysdCases8.js` — CDN, Log Aggregation (ELK), Time-Series DB, Workflow/Task Engine, Data Warehouse/OLAP, Media Processing Pipeline, Secrets Manager, Multi-Region DB. Wired into `seed:lld-all` (diagrams LAST) / `seed:sysd-all`. `audit:lld` 108/108 PASS, `audit:sysd` 99/99 PASS. Remaining to match AlgoMaster: ~65 LLD + ~78 SD.
- **LLD M15 + SD M14 (2026-06-05, local; not yet pushed/deployed):** another batch per track. **LLD → 15 modules / 116 topics / 352 exercises / 11920 XP:** M15 `seedLldCases9.js` — Thread Pool/Executor, ORM/Data Mapper (Identity Map + Unit of Work), SQL Query Builder, Diff Tool (LCS/Myers), Template Engine, Regex Matcher (NFA), Monopoly, 2048. **System Design → 14 modules / 107 topics / 323 exercises / 11520 XP:** M14 `seedSysdCases9.js` — Reddit/HN hot-ranking, Real-Time Bidding ad exchange, OTP/2FA service, push-notification gateway, offline-first sync, matchmaking, bulk-email sender, ML feature store. Wired into `seed:lld-all` (diagrams LAST) / `seed:sysd-all`. `audit:lld` 116/116 PASS, `audit:sysd` 107/107 PASS. Remaining to match AlgoMaster: ~57 LLD + ~70 SD.
- **LLD M16 + SD M15 (2026-06-05, local; not yet pushed/deployed):** another batch per track. **LLD → 16 modules / 124 topics / 376 exercises / 12810 XP:** M16 `seedLldCases10.js` — Event Bus/EventEmitter, Retry/Backoff Utility, Memoization Cache, CSV Parser, State-Machine Framework, Build System/Task Runner, Maze Generator+Solver, Scrabble. **System Design → 15 modules / 115 topics / 347 exercises / 12425 XP:** M15 `seedSysdCases10.js` — Social Graph/PYMK, Shopping Cart & Checkout, Subscription Billing, API Gateway, Container Orchestrator, Calendar/Scheduling, Stream Processing, Distributed Web Cache/Reverse Proxy. Wired into `seed:lld-all` (diagrams LAST) / `seed:sysd-all`. `audit:lld` 124/124 PASS, `audit:sysd` 115/115 PASS. Remaining to match AlgoMaster: ~49 LLD + ~62 SD.
- **LLD M17 (Data Structures) + SD M16 (2026-06-05, local; not yet pushed/deployed):** another batch per track. **LLD → 17 modules / 132 topics / 400 exercises / 13700 XP:** M17 `seedLldCases11.js` — designing the classic data structures: Heap/Priority Queue, Disjoint Set (Union-Find), Skip List, Circular Buffer, Bloom Filter, Graph, Balanced BST (AVL/Red-Black), Hash Map. **System Design → 16 modules / 123 topics / 371 exercises / 13335 XP:** M16 `seedSysdCases11.js` — SSO/Identity Provider, Change Data Capture, Service Mesh, Serverless/FaaS, Distributed SQL (NewSQL), Multi-Tenant SaaS, Backup & DR, Developer API Platform. Wired into `seed:lld-all` (diagrams LAST) / `seed:sysd-all`. `audit:lld` 132/132 PASS, `audit:sysd` 123/123 PASS. Remaining to match AlgoMaster: ~41 LLD + ~54 SD.
- **LLD M18 + SD M17 (2026-06-05, local; not yet pushed/deployed):** another batch per track. **LLD → 18 modules / 140 topics / 424 exercises / 14585 XP:** M18 `seedLldCases12.js` — Sliding Puzzle+A* solver, Othello/Reversi, Spell Checker, Package Dependency Resolver, Cron Expression Scheduler, URL Router, Middleware Pipeline, Serializer/Object Mapper. **System Design → 17 modules / 131 topics / 395 exercises / 14230 XP:** M17 `seedSysdCases12.js` — Service Discovery, Schema Registry, Audit Log/Compliance Trail, GDPR Data-Deletion, Image Processing CDN, Q&A Platform, E-Signature Platform, Uptime Monitoring. Wired into `seed:lld-all` (diagrams LAST) / `seed:sysd-all`. `audit:lld` 140/140 PASS, `audit:sysd` 131/131 PASS. Remaining to match AlgoMaster: ~33 LLD + ~46 SD.
- **LLD M19 (Advanced Data Structures) + SD M18 (AI/ML & Data Infra) (2026-06-05, local; not yet pushed/deployed):** **LLD → 19 modules / 148 topics / 448 exercises / 15470 XP:** M19 `seedLldCases13.js` — B+Tree, LSM-Tree, Consistent Hashing Ring, Quadtree, Segment Tree, Fenwick Tree (BIT), Interval Tree, Rope. **System Design → 18 modules / 139 topics / 419 exercises / 15130 XP:** M18 `seedSysdCases13.js` — Vector DB/Semantic Search, RAG Pipeline, LLM Inference Serving, ML Model Registry/Deployment, Distributed Model Training, PII Anonymization Pipeline, Distributed Graph DB, Geo Load Balancing (GSLB). Wired into `seed:lld-all` (diagrams LAST) / `seed:sysd-all`. `audit:lld` 148/148 PASS, `audit:sysd` 139/139 PASS. Remaining to match AlgoMaster: ~25 LLD + ~38 SD.
- **LLD M20 (Concurrency in Depth) + SD M19 (2026-06-05, local; not yet pushed/deployed):** **LLD → 20 modules / 156 topics / 472 exercises / 16355 XP:** M20 `seedLldCases14.js` — Counting Semaphore, Future/Promise, Event Loop, CountDownLatch/Barrier, Lock-Free Stack (CAS/ABA), Token-Bucket Rate Limiter, Dining Philosophers, Actor System. **System Design → 19 modules / 147 topics / 443 exercises / 16010 XP:** M19 `seedSysdCases14.js` — Saga/Transaction Orchestrator, Music Streaming (Spotify), Distributed File System (GFS/HDFS), Survey/Forms Platform, Package Registry, Captcha/Bot Detection, Two-Sided Marketplace, Real-Time OLAP (Druid/Pinot). Wired into `seed:lld-all` (diagrams LAST) / `seed:sysd-all`. `audit:lld` 156/156 PASS, `audit:sysd` 147/147 PASS. Remaining to match AlgoMaster: ~17 LLD + ~30 SD.
- **LLD M21 (Classic Games II) + SD M20 (Domain Platforms) (2026-06-05, local; not yet pushed/deployed):** **LLD → 21 modules / 164 topics / 496 exercises / 17205 XP:** M21 `seedLldCases15.js` — Blackjack, Wordle, Mastermind, Yahtzee, Boggle, Hangman, Memory/Concentration, Lights Out. **System Design → 20 modules / 155 topics / 467 exercises / 16895 XP:** M20 `seedSysdCases15.js` — Online Learning Platform, Plagiarism Detection, Credit Scoring, Stock Market Data Feed, Last-Mile Routing, Loyalty/Rewards, KYC/Identity Verification, Sportsbook. Wired into `seed:lld-all` (diagrams LAST) / `seed:sysd-all`. `audit:lld` 164/164 PASS, `audit:sysd` 155/155 PASS. Remaining to match AlgoMaster: ~9 LLD + ~22 SD.
- **LLD M22 (Games & Puzzles III) + SD M21 (Domain Platforms II) (2026-06-05, local; not yet pushed/deployed):** **LLD → 22 modules / 172 topics / 520 exercises / 18075 XP — AT PARITY with AlgoMaster's 173:** M22 `seedLldCases16.js` — Uno, Checkers, Chess Clock, Word Ladder Solver, Trivia/Quiz Engine, Mancala, Sudoku Generator, Crossword Generator/Solver. **System Design → 21 modules / 163 topics / 491 exercises / 17785 XP:** M21 `seedSysdCases16.js` — Restaurant Reservation (OpenTable), Helpdesk/Ticketing, CRM, News/RSS Aggregator, Photo Storage (Google Photos), Supply-Chain Track-and-Trace, Insurance Claims, Crowdfunding. Wired into `seed:lld-all` (diagrams LAST) / `seed:sysd-all`. `audit:lld` 172/172 PASS, `audit:sysd` 163/163 PASS. LLD count gap essentially CLOSED (172 vs 173); SD ~14 to go.
- **SD M22 (Networking & Security Infra) + M23 (Data & Platform Infra) (2026-06-05, local; deploy pending):** closes SD parity. **System Design → 23 modules / 179 topics / 539 exercises / 19570 XP — EXCEEDS AlgoMaster's 177.** M22 `seedSysdCases17.js` — DNS Service, Load Balancer (L4/L7), DDoS Protection, WAF, Certificate Authority/PKI, KMS, Zero-Trust Access, Cloud Pub/Sub. M23 `seedSysdCases18.js` — Data Lake/Lakehouse, Collaborative Whiteboard at scale, Geospatial DB, Blockchain/Distributed Ledger, ETL Orchestration, Quota/Metering, Anomaly Detection, Inbound Webhook Ingestion. Wired into `seed:sysd-all`. `audit:sysd` 179/179 PASS. **BOTH tracks now at/above AlgoMaster parity (LLD 172 vs 173, SD 179 vs 177).**
- **SD DEPTH standard — 143/143 case studies DEEP (2026-06→2026-08-24, live on prod):** count parity ≠ depth parity (topics were ~500 words + MCQ, 0 real diagrams vs AlgoMaster's ~5k-word illustrated articles), so depth is now a gated standard: `SD_DEPTH_STANDARD.md` defines DEEP (full interview framework in `teaching.blocks` tagged with `section` — requirements / estimation-with-numbers / hld / api / data_model / ≥2 deep_dives / bottlenecks — plus authored architecture SVG, ≥1,200 words, ≥3 graded exercises, ≥3 commonGaps + interviewRelevance) and `config/auditSysdDepth.mjs` (`npm run audit:sysd-depth`, supports `--require id,id`) gates it (PARTIAL = hard fail). Seeded via `seedSysdDepthPilot.js` + `seedSysdDepthBatch2–24.js` (`npm run seed:sysd-depth-pilot`..`-24`), each deepening ~6 case studies with authored box-and-arrow SVGs. **Batch 24 (2026-08-24) finished the final six** — Survey/Forms Platform, CRM (Salesforce, metadata-driven multi-tenant), News/RSS Aggregator, Supply-Chain Track-and-Trace, Insurance Claims Processing, Crowdfunding (Kickstarter) — **143/143 DEEP, 0 partial, 0 shallow; SD track now 179 topics / 968 exercises / 29,810 XP; `audit:sysd` + `audit:sysd-depth` PASS local + prod.**
- **Track-2 Pattern Quiz (2026-06-03):** "which pattern fits?" MCQ — `GET /v1/pro/tracks/:trackSlug/pattern-quiz?n=` (`getPatternQuiz` svc samples real LC-tagged problems, builds 4-choice questions from the `pattern` tags; `patternQuizQuerySchema`; `proGetPatternQuiz` api). `ProPatternQuiz` page at `/pro/:trackSlug/pattern-quiz` (instant feedback + score), linked from the Pattern Atlas header. Unit-tested (`proPractice.service.test.js` now 7/7).
- **Track-2 code-template library (2026-06-03):** `data/proCodeTemplates.js` — Java skeletons for 26 patterns (~34 templates) keyed by the `pattern` tag; `components/pro/TemplatePanel.jsx` (collapsible, Copy + Insert-into-editor) surfaced in `ProExerciseRunner` for the exercise's pattern. Frontend-only, no backend.
- **Track-2 Focus mode + status (2026-06-03):** distraction-free Focus toggle in `ProExerciseRunner` (hides the problem column, widens the editor); **Solved ✓** badges + "(x/N solved)" heading on `ProTopicView` via `proGetProgress`. Completes Track-2 (DSA Animator parity) — all 7 items done; see `DSA_ANIMATOR_GAP_CHECKLIST.md`.
- **Pro visual_aid renderer (2026-06-03):** `components/pro/VisualAid.jsx` turns `teaching.visual_aid` (previously rendered as the raw illustrator brief) into an actual visual — best-first: authored inline **`block.svg`** → auto-parsed **arrow-flow** pipeline (boxes + keyword icons, covers ~114 pro topics with no re-authoring) → styled fallback. **Every pro_java visual_aid (172) now renders as a visual: 114 auto-flow + 58 authored SVGs, 0 prose left.** SVGs seeded by `config/seedProVisualAidSvgs.js` (3 spatial: primitives/precedence/call-stack) + `config/seedProVisualAidSvgsBatch.js` (55 via reusable panels/grid/tree generators) + `seedProVisualAidSvgsBatch2.js` (31 structural: Throwable tree, decision/list traces). **Shape-faithful re-authoring (2026-06-03):** `seedProVisualAidFlowcharts.js` — 5 true **flowcharts with diamond decision boxes** (m1_t4 if/else/switch, m1_t5 for/while/do-while loops, m30_t5/m30_5_t5/m5_t5 decision cascades), fixing topics whose brief said "diamond" but rendered as rectangles. `seedProVisualAidShapes.js` — 8 literal-shape diagrams: **state machines** (m19_t1/m15_t4 circuit breaker, m10_t1 thread lifecycle, m24_t3 saga+compensation), **curve graphs** (m29_t1 six Big-O curves, m25_t4 load/stress/soak, m3_t5 O(N²) vs O(N)), **Venn** (m9_t3 Number ∩ Comparable). `seedProVisualAidScanner.js` — m1_t6 Scanner stdin→buffer→newline-bug→fix. **Full one-by-one audit (2026-06-03):** every Java visual_aid checked brief-vs-render (`JAVA_VISUAL_AID_AUDIT.md`); `seedProVisualAidBatch3.js` re-authors **all 59 remaining mismatches** as faithful diagrams (call-stacks, memory cells, hierarchy/recursion trees, state machines, decision flowcharts, grids/matrices, Big-O curves, bucket/heap/linked-list/queue structures) — each authored from its own brief, render-verified. **M34–M51 coverage (2026-06-03):** these modules were authored without a `visual_aid` (they have `concept_explanation` + `reflection`). `seedProVisualAidBatch4.js` now adds the full `teaching.visual_aid` object to all **73 diagram-worthy** topics (DSA: hash tables, trees/BST/trie, heaps, graphs, sorting, binary search, backtracking, DP, greedy, bitwise, recursion/D&C; plus system design, modern Java, engineering hygiene, and structural interview/communication topics). 19 pure-narrative topics (behavioral stories, mock-timing, company prep) intentionally skipped — see `JAVA_VISUAL_AID_AUDIT_M34.md`. Every pro_java topic that warrants a diagram now has one. **Panel→diagram fix (2026-06-03):** `seedProVisualAidBatch5.js` re-authors the **34** topics that had a generic early-batch "text-in-boxes" SVG even though their brief described a real diagram (flow/arrows/before-after) — e.g. m2_t1 method-call flow, m9_t1 generics, m10_t2 race/lock/volatile, m21_t1–t5 infra pipelines, m23_t1 JWT flow, m26_t2 RAG, m27_t3 DataLoader, m31_t5 LCS table. Root cause of the recurring miss: the audit had treated "has an svg" as "has a real diagram" and bulk-dismissed them; the real test is brief-vs-rendered-shape. **Audit rule going forward: a topic is only "done" if the SVG draws the shape the brief describes — not merely that an svg exists.** Extend via `$set teaching.visual_aid.svg`.
