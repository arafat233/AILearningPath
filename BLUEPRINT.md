...
- **Track-2 step-by-step animation player (2026-06-03):** `ProExercise.animation` (Mixed) + `components/pro/StepPlayer.jsx` (shared shell + per-`kind` stages) in `ProExerciseRunner`. 3 renderers — **array-pointers** (two-pointers, sliding-window), **stack** (monotonic-stack), **grid** (BFS/DFS) — across **21 flagship problems** authored by `config/seedAnimation{TwoPointers,SlidingWindow,MonotonicStack,Grid,Batch2}.js` (batch2 adds Sort Colors, Trapping Rain Water, Search Insert, Valid Parens, Eval RPN, Min Stack, 01 Matrix, Surrounded Regions). Play/pause/step/scrub. Build + API verified (browser pixel QA pending).
