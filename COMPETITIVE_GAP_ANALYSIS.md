...
- **System Design topic COUNT (179) exceeds AlgoMaster's 177** — every topic has graded exercises + flow diagrams, plus a modern **AI/ML-infra** module AlgoMaster lacks. ⚠️ **COUNT ≠ DEPTH:** AlgoMaster's case-study articles are ~4,500–5,000 words with capacity math, API, schema, and real architecture diagrams; our topics were ~500 words + an MCQ with no authored diagrams. This was count-metric drift (caught 2026-06-05). **Fix in flight:** `SD_DEPTH_STANDARD.md` + `auditSysdDepth.mjs` now gate on depth; 29 flagship case studies are now at full depth (≥1.2k words, full framework, real SVGs, render-verified): URL shortener, rate limiter, news feed, chat, KV store, Instagram, video streaming, ride-sharing, web crawler, distributed cache, typeahead, search engine, message queue, leaderboard, notifications, payment system, maps routing, file sync, unique-id generator, object store (S3), proximity service, distributed lock, job scheduler, distributed counter, top-k/trending, CDN, log aggregation, metrics/monitoring, saga orchestrator — plus API gateway, SSO, CDC, service mesh, tracing, webhooks, service discovery, schema registry, audit log, multi-tenant SaaS, GDPR deletion, OTP/2FA, vector DB, RAG, LLM serving, MLOps registry, load balancer, DNS, DDoS, WAF, CA/PKI, KMS, pub/sub, secrets manager, data warehouse, lakehouse, ETL orchestration, real-time OLAP, anomaly detection, stream processing, container orchestrator, backup/DR, collaborative whiteboard, geospatial DB, blockchain/ledger, image-processing CDN, stock brokerage, photo storage, music streaming, distributed file system, video conferencing, live streaming, IoT platform, multiplayer game server, hot ranking (Reddit/HN), RTB ad exchange, content moderation, ad-click aggregator, Twitter (timeline fanout), Google Docs (collaborative editing), stock exchange (matching engine), flash sale, online code judge, Airbnb/Booking, recommendation system, email service (Gmail), ticket booking (Ticketmaster), pastebin, live presence, Slack, time-series DB, multi-region DB, distributed SQL (NewSQL), serverless/FaaS, workflow engine, media processing pipeline, A/B testing platform, fraud detection, Tinder (matching), comment system, live location sharing, CI/CD pipeline, feature-flag/config service, push notification gateway, social graph (PYMK), shopping cart & checkout, subscription billing, matchmaking, feature store (ML), calendar/scheduling, distributed web cache/reverse proxy, offline-first sync, Q&A platform, distributed graph DB, uptime/status monitoring, two-sided marketplace, package registry, captcha/bot detection, stock market data feed, last-mile delivery routing, developer API platform, e-signature, zero-trust access, webhook ingestion gateway, quota/usage-metering, distributed model training — **DEEP coverage 125/143 case studies (~87%)** (incl. AI/ML-infra, security/networking-infra, data-infra, platform-infra, and media/real-time sets AlgoMaster lacks); the remaining 18 are shallow until deepened. Do NOT claim depth parity from the topic count. (2026-06-06)
