...
| UX-1 | ✅ done | `a69abd08` | **Pro-track UX polish + performance (2026-05-30).** Backend: proAnalyticsService .toObject() crash fix; authController register passes grade/examBoard as null to skip schema defaults; userRoutes Redis cache /user/me (30s) + /user/nav (60s) with parallel queries + cache invalidation on writes; proService Redis cache getTopic + listExercises (5 min TTL — warm reads ~5ms vs ~200ms). Frontend: Analytics pro-aware; Certificate track-branched (purple XP cert for pro, gold grade cert for school); App.jsx BoardGated skips grade check for pro; Layout sidebar subtitle "Java · Professional" for pro; Profile shows "Java · Professional" + hides school fields for pro; Planner create-plan modal track-aware; VisualizerShell all 45 sandboxes lazy-loaded with SortingSandbox extracted to its own file; ProTopicView prefetches VisualizerShell in parallel with topic API; vite.config server.warmup + optimizeDeps.include. |
| UX-2 | ✅ done | `9d44bc65` | **Track switching + route switches (2026-05-30).** TrackSwitcher + TrackTabs: sidebar + dashboard UI for switching between school and pro. DashboardSwitch / PracticeSwitch / BookmarksSwitch: route-level switch components that render pro or school version based on activeTrack. Dashboard: pro-aware hero + nav tiles. trackStore: hydration flag, refreshNav guard, setActiveTrack via API. |
