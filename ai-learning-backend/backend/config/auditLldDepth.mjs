/**
 * auditLldDepth.mjs — DEPTH audit for the System Design track (pro_lld).
 *
 ...
 * depth. This audit gates on DEPTH per LLD_DEPTH_STANDARD.md so the drift can't
 ...
 *
 * Usage:
 *   node config/auditLldDepth.mjs
 *   node config/auditLldDepth.mjs --require sysd_m1_t10,sysd_m1_t11
...
const REQUIRED_SECTIONS = ["requirements", "entities", "design", "code", "tradeoffs"];
...
async function run() {
  ...
  console.log(`\n=== LLD DEPTH AUDIT (${TRACK_KEY}) ===`);
  console.log(`case-study topics: ${caseStudies.length}  (concept/fundamentals topics excluded)`);
  console.log(`\nDEEP COVERAGE: ${deep.length}/${caseStudies.length} case studies meet LLD_DEPTH_STANDARD.md`);
