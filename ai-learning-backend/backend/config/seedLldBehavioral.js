...
const TOPICS = [
  T("lld_m4_t1", 1, "Command Pattern", "command-pattern",
    ...
    [
      ...
      { kind: "concept", heading: "Where you'll see it",
        body: "java.lang.Runnable is a Command — a packaged unit of work you hand to a thread or ExecutorService to run later; Swing/JavaFX Action objects bind a menu/button to a command; editors keep an undo stack of command objects; task/job queues serialize commands to run on workers. At architecture scale, CQRS and command buses route command objects to handlers. The signature: behaviour captured as a first-class object you store, queue, log, or reverse." },
    ],
  ...
  T("lld_m4_t2", 2, "State Pattern", "state-pattern",
    ...
    [
      ...
      { kind: "concept", heading: "Where you'll see it",
        body: "Any real finite-state machine: a TCP connection (Closed/Listen/Established), an order lifecycle (Placed→Paid→Shipped→Delivered), a document workflow (Draft→Review→Published), a media player (Playing/Paused/Stopped), a vending machine. Spring Statemachine and XState (JS) formalize it. The tell that it's State and not just a status field: behaviour changes with the mode AND the modes have defined legal transitions you want to enforce." },
    ],
  ...
  T("lld_m4_t3", 3, "Template Method Pattern", "template-method-pattern",
    ...
    [
      ...
      { kind: "concept", heading: "Where you'll see it",
        body: "Frameworks live on Template Method: JUnit calls your @BeforeEach/test/@AfterEach in a fixed order; the Servlet lifecycle calls init()/service()/destroy(); Spring's AbstractController and JdbcTemplate fix the flow and call your callbacks; java.util.AbstractList implements the list operations in terms of the few methods you override (get/size). It's the dominant 'framework calls your code' pattern — you fill in the steps, the base owns the algorithm." },
    ],
  ...
  T("lld_m4_t6", 6, "Mediator Pattern", "mediator-pattern",
    ...
    [
      ...
      { kind: "concept", heading: "Where you'll see it",
        body: "A chat room routing messages between users is the textbook example; UI dialog controllers coordinating widgets; air-traffic control as the analogy. In Spring, ApplicationEventPublisher acts as a mediator/event hub between beans. At service scale, a message broker plays the mediator role so services interact through it rather than calling each other directly — turning an N² mesh of dependencies into a hub-and-spoke." },
    ],
  ...
  T("lld_m4_t7", 7, "Memento Pattern", "memento-pattern",
    ...
    [
      ...
      { kind: "concept", heading: "Where you'll see it",
        body: "Undo/redo in editors and IDEs; game save-states and checkpoints; database savepoints and transaction rollback (a conceptual cousin); the Ctrl-Z stack in any document app. Memento keeps the saved state OPAQUE to the caretaker (the undo manager stores tokens it can't read), which is what separates it from naively copying public fields out. For big state, real systems snapshot deltas or periodic checkpoints rather than full copies every keystroke." },
    ],
  ...
  T("lld_m4_t8", 8, "Visitor Pattern", "visitor-pattern",
    ...
    [
      ...
      { kind: "concept", heading: "Where you'll see it",
        body: "Compilers and interpreters walk a fixed AST with many operations (type-check, optimize, generate code, pretty-print) — each is a visitor. java.nio.file.Files.walkFileTree uses a FileVisitor; annotation processors and the javax.lang.model API visit program elements; XML/JSON document processors visit nodes. The pattern shines exactly where the node types are stable but you keep needing new passes over them." },
    ],
