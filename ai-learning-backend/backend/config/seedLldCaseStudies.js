...
const TOPICS = [
  T("lld_m5_t1", 1, "Design an Elevator System", "design-elevator",
    ...
    [
      ...
      { kind: "concept", heading: "Common interview follow-ups",
        body: "'Now there are 4 elevators — how do you pick which one serves a hall request?' (the SchedulingStrategy: nearest-car, then SCAN/LOOK to minimise travel). 'Add express elevators / a freight elevator' (subtypes + eligibility rules). 'Handle capacity and weight limits' (reject boarding, keep serving stops). 'Maintenance/emergency mode' (a State that suspends normal dispatch). 'Make it concurrent' (one thread per car + a thread-safe request queue). Lead by restating which non-functionals are in scope before coding them." },
    ],
  ...
  T("lld_m5_t2", 2, "Design a Vending Machine", "design-vending-machine",
    ...
    [
      ...
      { kind: "concept", heading: "Common interview follow-ups",
        body: "'Support card/UPI payments, not just coins' (a PaymentStrategy alongside the State machine — payment is orthogonal to the dispense states). 'Make change with the fewest coins' (greedy/DP over denominations, refuse if impossible). 'Add an admin/restock mode' (another State + role check). 'Track inventory and alert on low stock' (Observer on the inventory). 'Two people insert money at once' (per-machine state isn't shared, but if modelling one shared machine, guard the transition atomically)." },
    ],
  ...
  T("lld_m5_t3", 3, "Design Splitwise (Expense Sharing)", "design-splitwise",
    ...
    [
      ...
      { kind: "concept", heading: "Common interview follow-ups",
        body: "'Support multiple currencies' (store amounts with a currency code; settle per currency or convert at a rate). 'Groups vs 1:1 friendships' (a Group aggregates members; balances still net pairwise). 'Partial settlements / record a payment' (apply a payment that reduces a pairwise balance). 'Show an activity feed & notify on new expenses' (Observer / notification service). 'Prove the settle-up is minimal' (it's NP-hard in general; greedy is the accepted practical answer — say so)." },
    ],
  ...
  T("lld_m5_t4", 4, "Design a Rate Limiter", "design-rate-limiter",
    ...
    [
      ...
      { kind: "concept", heading: "Common interview follow-ups",
        body: "'Make it work across 50 servers' (shared Redis + an atomic check-and-decrement via a Lua script — the crux). 'Smooth the fixed-window boundary burst' (sliding-window-log or sliding-window-counter). 'Different limits per tier' (per-user, per-IP, per-endpoint keys, possibly composed). 'What if Redis is down?' (fail-open for availability vs fail-closed for protection — a deliberate choice). 'Where does it live?' (at the API gateway, as middleware — the LLD Chain-of-Responsibility)." },
    ],
  ...
  T("lld_m5_t6", 6, "Design Snake & Ladder", "design-snake-and-ladder",
    ...
    [
      ...
      { kind: "concept", heading: "Common interview follow-ups",
        body: "'Configurable board (size, snake/ladder positions)' (inject the jump map + size). 'Loaded or multiple dice' (the Dice Strategy already enables this; sum N dice). 'Persist and resume a game' (snapshot player positions + whose turn — a Memento). 'Rule variants' (exact roll to finish, extra turn on a six). 'Many concurrent games' (each Game is independent state; a GameManager keyed by gameId). The reusable insight stays: snakes and ladders are one jump abstraction in a Map." },
    ],
  ...
  T("lld_m5_t7", 7, "Design a Notification Service", "design-notification-service",
    ...
    [
      ...
      { kind: "concept", heading: "Common interview follow-ups",
        body: "'Respect user preferences & quiet hours' (a preferences service consulted before sending). 'Don't double-send on retry' (idempotency key + a dedup store with TTL). 'Collapse 5 likes into one digest' (batching/aggregation). 'Add WhatsApp/Slack' (a new Channel adapter — Strategy, one class). 'Track delivered/opened' (per-message status + webhooks from providers). 'Priority' (OTP before marketing → priority queues). This is where LLD (Strategy/Factory/Observer) meets system design (queues, dedup at scale)." },
    ],
  ...
  T("lld_m5_t8", 8, "Design a Logging Framework", "design-logging-framework",
    ...
    [
      ...
      { kind: "concept", heading: "Common interview follow-ups",
        body: "'Make logging async so it doesn't slow requests' (a lock-free ring buffer + a background writer thread — Log4j2's AsyncAppender / LMAX Disruptor). 'Per-package log levels' (a hierarchical level lookup, e.g. com.app.payments=DEBUG). 'Structured/JSON logs for machines' (a JsonFormatter Strategy). 'Rotate files by size/date' (a rolling appender). 'Correlate logs across a request' (MDC / trace IDs). Real frameworks (Log4j2, Logback, SLF4J) combine Chain-of-Responsibility (levels), Strategy (appenders/formatters), and a careful Singleton/factory." },
    ],
