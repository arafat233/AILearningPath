...
const TOPICS = [
  {
    ...
    teaching: { blocks: [
      { kind: "concept", heading: "The four pillars are design levers, not trivia",
        body: "Object-oriented design rests on four pillars: Encapsulation (protect data behind methods), Abstraction (expose the what, hide the how), Inheritance (reuse via an is-a hierarchy), and Polymorphism (one call site, many behaviours). Junior engineers can define them; the LLD signal is choosing the right one for a problem — and knowing when each hurts. We'll take them one at a time with code, then the decision that matters most: composition vs inheritance." },
      { kind: "concept", heading: "Encapsulation — protect the invariants",
        body: "Encapsulation means the object owns its data and the only way to change it is through methods that keep the object valid. The class becomes the single guardian of its own rules (its invariants), so no caller can ever put it in an illegal state. Public mutable fields scatter that responsibility across every caller — and every bug." },
      { kind: "code", heading: "Encapsulation in Java",
        body: "class BankAccount {\n    private long balanceCents;            // nobody can touch this directly\n    public void deposit(long c) {\n        if (c <= 0) throw new IllegalArgumentException(\"amount must be positive\");\n        balanceCents += c;\n    }\n    public void withdraw(long c) {\n        if (c > balanceCents) throw new IllegalStateException(\"insufficient funds\"); // the invariant\n        balanceCents -= c;\n    }\n    public long balanceCents() { return balanceCents; } // read-only view\n}\n// The 'never negative' rule lives in ONE place. A public field would force every caller to re-check it." },
      { kind: "concept", heading: "Abstraction — depend on the what, not the how",
        body: "Abstraction exposes a small, stable surface (an interface or abstract method) while hiding the implementation behind it. Callers of `List` don't care whether it's an ArrayList or LinkedList. Every place you can program to an interface is a place you can later swap the implementation without touching callers — the foundation of testability (inject a fake) and of the Dependency Inversion Principle." },
      { kind: "concept", heading: "Polymorphism — the engine of extensibility",
        body: "Polymorphism lets one call site invoke different behaviour depending on the runtime type: `shape.area()` runs Circle.area() or Square.area() without the caller knowing which. This is what makes the Open/Closed Principle possible — add a new Shape subclass and existing loops keep working, because they only ever call the abstract method. Most design patterns (Strategy, State, Template Method, Command…) are polymorphism applied with intent." },
      { kind: "code", heading: "Polymorphism over a type switch",
        body: "// Anti-pattern: a switch that grows every time you add a shape\n// double area(Shape s) { if (s.type==CIRCLE) ...; else if (s.type==SQUARE) ...; }\n\ninterface Shape { double area(); }                 // the abstraction\nclass Circle implements Shape { double r; public double area(){ return Math.PI*r*r; } }\nclass Square implements Shape { double s; public double area(){ return s*s; } }\n\ndouble total(List<Shape> shapes) {                 // never edited when a new Shape is added\n    double sum = 0; for (Shape s : shapes) sum += s.area(); return sum;\n}" },
      { kind: "concept", heading: "Inheritance vs Composition — the decision that matters",
        body: "Inheritance models 'is-a' and reuses code by extending a base class — but it is the tightest coupling in OOP: the subclass depends on the parent's internals, you can only extend one class, and a change to the base ripples into every subclass (the 'fragile base class' problem). Composition models 'has-a' by holding a reference to a collaborator you delegate to. The industry rule — and the answer interviewers want — is 'favour composition over inheritance': it's looser, swappable at runtime, and not limited to one parent." },
      { kind: "code", heading: "Composition over inheritance",
        body: "// Inheritance: a Car IS an Engine? No. And you can't also extend Vehicle.\n// class Car extends Engine { ... }   // wrong relationship, single-parent lock-in\n\n// Composition: a Car HAS an Engine — swappable, testable, unlimited collaborators\nclass Car {\n    private final Engine engine;            // delegate to a collaborator\n    Car(Engine engine) { this.engine = engine; }  // inject it (also enables testing)\n    void start() { engine.ignite(); }\n}\n// Swap a PetrolEngine for an ElectricEngine without touching Car." },
      { kind: "concept", heading: "Where you'll see this in real code",
        body: "Java's own libraries are a masterclass: `List`/`Map` are abstraction (program to the interface); `java.io` streams use composition (a BufferedReader wraps a Reader — see the Decorator pattern); Spring beans are injected collaborators (composition + DIP), not extended. When you sketch a design, prefer an interface + injected implementations; reach for inheritance only for genuine is-a hierarchies where the base is stable and you control it." },
    ] },
...

  {
    ...
    teaching: { blocks: [
      ...
      { kind: "code", heading: "OCP: edit-the-switch vs add-a-class",
        body: "// VIOLATES OCP — every new method edits this switch\nint fee(String type, int amt) {\n    if (type.equals(\"CARD\")) return amt + 30;\n    else if (type.equals(\"UPI\")) return amt;        // add WALLET? edit here again\n    throw new IllegalArgumentException(type);\n}\n\n// OBEYS OCP — a new method is a new class, this code never changes (Strategy)\ninterface Payment { int fee(int amt); }\nclass Card implements Payment { public int fee(int a){ return a + 30; } }\nclass Upi  implements Payment { public int fee(int a){ return a; } }\nint checkout(Payment p, int amt) { return p.fee(amt); }" },
      { kind: "code", heading: "DIP: inject the abstraction",
        body: "// VIOLATES DIP — welded to MySQL, impossible to unit-test\nclass OrderService {\n    private final MySqlOrderRepo repo = new MySqlOrderRepo();   // hard-wired concretion\n}\n\n// OBEYS DIP — depends on an interface, implementation injected (swap / mock freely)\ninterface OrderRepo { void save(Order o); }\nclass OrderService {\n    private final OrderRepo repo;\n    OrderService(OrderRepo repo) { this.repo = repo; }          // constructor injection\n}\n// Prod: new OrderService(new PostgresOrderRepo());  Test: new OrderService(new InMemoryOrderRepo());" },
      { kind: "concept", heading: "Spotting & fixing violations live (the interview move)",
        body: "Interviewers hand you smelly code and watch you name the principle + refactor. Pattern-match: a growing if/else over a type → OCP, fix with Strategy/polymorphism. A class whose name needs 'and' → SRP, split it. A subclass that throws on a base method or changes inherited behaviour → LSP, rethink the hierarchy (prefer composition). A `new ConcreteThing()` inside business logic → DIP, inject it. An interface implementers half-stub → ISP, split the interface." },
      { kind: "concept", heading: "Don't over-apply it",
        body: "SOLID buys changeability, but each split adds a class and indirection. A two-field value object doesn't need an interface; a script that runs once doesn't need DIP. Apply a principle when you can name the change it absorbs ('payment methods will grow → OCP'); otherwise you trade real simplicity for imagined flexibility (YAGNI). Frameworks like Spring lean hard on DIP because their whole job is wiring swappable beans — your app may need it more lightly." },
    ] },
...

  {
    ...
    teaching: { blocks: [
      ...
      { kind: "code", heading: "From diagram to code",
        body: "// Order \"1\" ──◆ \"1..*\" LineItem   (composition: items die with the order)\n// Order \"*\" ──── \"1\" Customer      (association: order references a customer)\nclass Order {\n    private final Customer customer;              // association (1)\n    private final List<LineItem> items;           // composition, multiplicity 1..*\n    Order(Customer c, List<LineItem> items) {\n        if (items.isEmpty()) throw new IllegalArgumentException(\"order needs >=1 item\"); // the 1..* invariant\n        this.customer = c; this.items = List.copyOf(items);\n    }\n}\n// The diamond/multiplicity isn't decoration — it dictated the field type, ownership, and the guard." },
      { kind: "concept", heading: "Beyond class diagrams",
        body: "Class diagrams show static structure, but LLD interviews sometimes want behaviour too. A SEQUENCE diagram shows the ordered messages between objects for one scenario (great for explaining a flow like 'place order'). A STATE-MACHINE diagram shows an object's states and transitions (perfect for a vending machine or order lifecycle — see the State pattern). You rarely need use-case or activity diagrams in coding rounds; class + a quick sequence diagram cover most discussions." },
    ] },
...

  {
    ...
    teaching: { blocks: [
      ...
      { kind: "code", heading: "Real-world: Comparator IS Strategy",
        body: "// java.util.Comparator is the textbook Strategy — sort() delegates the\n// 'how to compare' algorithm to whichever comparator you pass.\nList<Player> players = ...;\nplayers.sort(Comparator.comparingInt(Player::score));        // strategy 1\nplayers.sort(Comparator.comparing(Player::name));            // strategy 2, swapped at runtime\nplayers.sort(Comparator.comparingInt(Player::score).reversed()\n                       .thenComparing(Player::name));        // composed strategy" },
      { kind: "concept", heading: "Where you'll see it & when to use it",
        body: "Real uses: Comparator (sorting), java.util.function interfaces passed to methods, Spring's many pluggable *Strategy beans, payment/shipping/pricing selectors, compression/encryption algorithm pickers. USE Strategy when you have two or more interchangeable algorithms selected at runtime or by config. DON'T bother for a single fixed algorithm — that's needless indirection. Trade-off: clients must know the strategies exist to choose one, and you add a class per algorithm." },
    ] },
...

  {
    ...
    teaching: { blocks: [
      ...
      { kind: "code", heading: "Safe notify (copy to avoid CME)",
        body: "void setPrice(int p) {\n    this.price = p;\n    // iterate a snapshot so an observer that unsubscribes during update()\n    // doesn't corrupt the loop (ConcurrentModificationException)\n    for (Observer o : new ArrayList<>(observers)) {\n        o.update(p);\n    }\n}" },
      { kind: "concept", heading: "Where you'll see it & scaling up",
        body: "Real uses: GUI event listeners (button.addActionListener), JavaBeans PropertyChangeListener, Spring's ApplicationEvent/@EventListener, and — at distributed scale — message brokers and pub/sub (Kafka, Redis pub/sub). Observer is in-process; when subjects and observers live on different machines you graduate to a message queue (same intent, network in between). USE it for one-to-many 'when X changes, others react' where the subject shouldn't know its listeners. Watch lifecycles (unsubscribe) and threading (notify off the critical path if observers are slow)." },
    ] },
...

  {
    ...
    teaching: { blocks: [
      ...
      { kind: "concept", heading: "Three factory flavours, don't conflate them",
        body: "Simple Factory = one method with a switch (not GoF, most common). Factory Method = subclasses override a creation method to choose the product (one product type, decided by which creator subclass). Abstract Factory = an object whose methods create a whole FAMILY of related products that must match (covered in the Creational module). Reach for the simplest that fits — usually a Simple Factory." },
      { kind: "concept", heading: "Where you'll see it",
        body: "The JDK is full of factories: Calendar.getInstance(), NumberFormat.getCurrencyInstance(), DriverManager.getConnection(url), List.of(...), Executors.newFixedThreadPool(n). Spring's whole container is a BeanFactory. In LLD case studies you'll write a VehicleFactory (parking lot), a ChannelFactory (notifications), a ShapeFactory (drawing app). The win is always the same: callers depend on the interface and the 'which concrete class' decision lives in exactly one place." },
    ] },
...

  {
    ...
    teaching: { blocks: [
      ...
      { kind: "code", heading: "Double-checked locking + the enum singleton",
        body: "// Double-checked locking: lazy, fast after init — but the field MUST be volatile\nclass Conn {\n    private static volatile Conn instance;          // volatile prevents a half-built object leak\n    private Conn() {}\n    static Conn get() {\n        if (instance == null) {                     // 1st check (no lock)\n            synchronized (Conn.class) {\n                if (instance == null) instance = new Conn();  // 2nd check (locked)\n            }\n        }\n        return instance;\n    }\n}\n\n// Enum singleton — Effective Java's recommendation: concise, serialization- & reflection-safe\nenum Config { INSTANCE; public void load() { /* ... */ } }   // Config.INSTANCE" },
      { kind: "concept", heading: "Where you'll see it & the modern stance",
        body: "Real singletons: Runtime.getRuntime(), a logger factory, a connection/thread pool. In Spring, the DEFAULT bean scope is effectively a singleton — but the container creates and injects it, giving you one instance WITHOUT the global-access-point downside. That's the modern move: keep the 'one instance' benefit, drop the hidden global. In an interview, write the holder/enum version, then volunteer 'in a real app I'd make this a DI-managed singleton instead of a static accessor.'" },
    ] },
...

  {
    ...
    teaching: { blocks: [
      ...
      { kind: "code", heading: "Real-world: java.io is a decorator chain",
        body: "// Each wrapper is a Decorator adding one capability, all sharing the Reader/InputStream interface\nReader r = new BufferedReader(            // adds buffering\n              new InputStreamReader(      // adds bytes→chars decoding\n                  new FileInputStream(\"data.txt\")));   // the concrete component\n// Same idea, output side:\nOutputStream o = new GZIPOutputStream(    // adds compression\n                    new BufferedOutputStream(\n                        new FileOutputStream(\"out.gz\")));" },
      { kind: "concept", heading: "Where you'll see it & when to use it",
        body: "Beyond java.io: HttpServletRequestWrapper (servlet filters wrap the request), Collections.unmodifiableList/synchronizedList (wrap a list to add a constraint), and UI toolkits (a ScrollDecorator around a component). USE Decorator when responsibilities should be added/removed at runtime and stack in any combination — it turns a 2^N subclass explosion into N small classes. DON'T use it when there's only one fixed add-on (just put it in the class) or when callers need to reach methods the wrapper hides." },
    ] },
...

  {
    ...
    teaching: { blocks: [
      ...
      { kind: "concept", heading: "Real-world & the distinction that trips people up",
        body: "JDK adapters: Arrays.asList(array) adapts an array to the List interface; InputStreamReader adapts a byte InputStream to a char Reader; Collections has several. Spring MVC's HandlerAdapter lets the dispatcher invoke wildly different controller types through one interface. The frequent interview trap — Adapter vs Facade vs Decorator: Adapter CHANGES an interface to one the client expects (compatibility); Facade SIMPLIFIES many subsystem classes behind one new interface (convenience); Decorator KEEPS the interface and ADDS behaviour (enhancement). All wrap; the intent differs." },
    ] },
