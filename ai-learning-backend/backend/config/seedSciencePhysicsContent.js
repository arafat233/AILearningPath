...
const TOPICS = [
  ...
  // ═══════════════════════════════════════════════════════════════════
  // CHAPTER 12 — Domestic Electric Circuits (Section 12.5)
  // ═══════════════════════════════════════════════════════════════════
  {
    topicId: "sci_ch12_domestic_circuits",
    subject: "Science",
    chapterNumber: 12,
    name: "Domestic Electric Circuits — Earth Wire, Fuse and MCB",
    prerequisite_knowledge: [
      "Ohm's law — current, voltage, resistance",
      "Series and parallel combinations of resistors",
      "Heating effect of current (Joule's law)",
      "Concept of electric power and overloading",
    ],
    key_formulas: [
      "Short circuit: R → 0, so I = V/R → very large; fuse wire melts",
      "Overloading: P = VI; too many appliances → I exceeds fuse rating",
      "Earth wire connects metal body of appliance to earth (0 V); provides safe discharge path",
    ],
    teaching_content: {
      intuition:
        "Think of your home's electrical wiring as a water distribution system with built-in safety valves. The live wire (pressure/hot) brings current in; the neutral wire takes it out. If something goes wrong — a pipe bursts (short circuit) or too many taps open at once (overloading) — the fuse acts like a pressure relief valve that blows and cuts off the flow. The earth wire is the emergency drain: if the metal body of an appliance accidentally becomes live, the earth wire carries that dangerous current harmlessly to the ground before it can electrocute you.",
      process_explanation:
        "THREE-WIRE SYSTEM IN DOMESTIC CIRCUITS:\n\nEvery domestic circuit uses three wires:\n1. LIVE WIRE (Phase wire) — colour: red (old) or brown (new). Carries current from the supply at ~220–240 V. This is the dangerous wire.\n2. NEUTRAL WIRE — colour: black (old) or blue (new). Returns current to the supply; normally at 0 V.\n3. EARTH WIRE — colour: green/yellow (new). Connected to a metal rod buried deep in the earth. All metal bodies of appliances are connected to the earth wire.\n\nWhy three wires?\n• Live + Neutral = sufficient for current to flow (the circuit is complete)\n• Earth wire = SAFETY only — it carries no current during normal operation\n\nEARTH WIRE AND ELECTRIC SHOCK PREVENTION:\nImagine a washing machine with a damaged insulation inside. The live wire accidentally touches the metal drum → drum becomes live at 220 V. If you touch the drum, current flows through your body to earth → fatal electric shock.\nWith an earth wire: the metal drum is connected to earth (0 V) via the earth wire. When the live wire touches the drum → a large current flows through the earth wire → the fuse blows → circuit breaks → drum is safe. The fuse blows before you can be harmed.\n\nSHORT CIRCUIT:\nA short circuit occurs when the live wire and neutral wire come into direct contact (e.g., due to damaged insulation, a loose connection, or a nail through a wire). The resistance of the circuit drops to nearly zero → using V = IR, if R ≈ 0, then I = V/R → extremely large current flows → wires heat up → fire risk. Fuse blows and breaks the circuit.\n\nOVERLOADING:\nOverloading occurs when too many high-wattage appliances (geyser, AC, microwave) are connected to the same circuit simultaneously. Total current drawn = (P₁ + P₂ + P₃)/V. If total current exceeds the fuse rating (e.g., 15 A), the fuse blows.\n\nFUSE:\n• Made of a low-melting-point alloy (lead-tin or tin-copper)\n• Placed in SERIES with the live wire — so excess current must pass through it\n• Melts when current exceeds its rating (3 A, 5 A, 13 A, or 15 A types)\n• One-time use — must be replaced after it blows\n• Always fits in the live wire, never in the neutral wire\n\nMCB (MINIATURE CIRCUIT BREAKER):\n• Modern replacement for fuses — increasingly used in new buildings\n• Operates on electromagnetic effect: large current → strong magnetic field → trips a switch automatically\n• Advantage over fuse: can be RESET (flicked back ON) after the fault is fixed; no replacement needed\n• Faster response than fuse; can handle higher currents more precisely\n• MCBs also protect against earth leakage (RCCB/ELCB variants)\n\nHOUSEHOLD CIRCUIT ARRANGEMENT:\n• All appliances in a house are connected in PARALLEL (not series)\n• Reason: each appliance gets the full mains voltage (220 V); switching one off doesn't affect others; individual switches work independently\n• A main switch and fuse/MCB at the meter board controls the entire supply",
      worked_example:
        "CBSE exam question: 'What is the function of the earth wire in a domestic circuit? Why is it necessary to earth metallic appliances?' Answer: The earth wire (green/yellow insulation) is connected to a thick metal plate or rod buried deep in the ground. In normal operation it carries no current. If the live wire inside an appliance accidentally touches its metal body (due to insulation failure), the metal body would become live at 220 V — dangerous to touch. The earth wire provides a low-resistance path from the metal body to earth (0 V). The resulting large current blows the fuse and disconnects the circuit before a person can receive an electric shock. It is necessary for metallic appliances (geysers, washing machines, refrigerators, irons) because their metal bodies could otherwise become live.\n\nSecond question: 'What is the difference between a fuse and an MCB? Why are MCBs preferred?' Answer: Fuse: a thin wire of low-melting-point alloy connected in series with the live wire; melts and breaks the circuit when current exceeds its rating; must be REPLACED after use. MCB: an electromagnetic switch; trips automatically when excess current flows; can be RESET by flicking it back ON after fixing the fault. MCBs are preferred because: (1) they respond faster; (2) they can be reset — no replacement needed; (3) they are safer and more reliable.\n\nThird question: 'Three appliances of 1000 W, 1500 W and 500 W are connected to a 220 V supply with a 15 A fuse. Will the fuse blow?' Answer: Total power = 1000 + 1500 + 500 = 3000 W. Total current I = P/V = 3000/220 ≈ 13.6 A. Since 13.6 A < 15 A (fuse rating), the fuse will NOT blow.",
      common_misconceptions: [
        "The earth wire carries current during normal operation — FALSE. The earth wire carries current ONLY when a fault occurs (metal body becomes live). In normal operation it carries zero current.",
        "The fuse should be connected to the neutral wire — FALSE. The fuse must always be in the LIVE wire. If connected to the neutral wire, the appliance remains at live potential even when the fuse blows, making it dangerous to touch.",
        "Short circuit means the circuit is incomplete — FALSE. Short circuit means the live and neutral wires touch DIRECTLY, creating a path of nearly zero resistance — the circuit is very much complete, but with excessive current.",
        "All appliances in a home are connected in series — FALSE. Domestic appliances are in PARALLEL so each gets full 220 V and each can be switched independently.",
        "MCB and fuse do the same thing in the same way — PARTIALLY FALSE. Both protect against overcurrent, but a fuse melts (needs replacement) while an MCB trips magnetically (just reset it).",
      ],
      shortcuts_and_tricks: [
        "Wire colours mnemonic: Live = Lethal = Red/Brown (danger colour). Neutral = Not dangerous = Black/Blue (cool/safe colour). Earth = Green = Grass/ground colour.",
        "FUSE = 'Fails Under Surge of Electricity' — it's in series with the LIVE wire; always melts on overcurrent.",
        "MCB vs Fuse exam trick: MCB = resettable (flip it back); Fuse = replaceable (buy a new one). Use this to distinguish advantages.",
        "Overloading vs Short circuit: Overloading = slow build-up (many appliances, I slightly over limit). Short circuit = sudden huge current (live + neutral touch, R → 0). Both blow the fuse, but short circuit is faster and more violent.",
        "Earth wire connects metal bodies; fuse protects the circuit wiring. Two different safety devices doing two different jobs — both essential.",
      ],
      diagram_description:
        "Diagram 1 (three-wire household circuit): A meter board on the left showing the main switch and fuse/MCB. Three wires emerge: Live (L, red/brown), Neutral (N, black/blue), Earth (E, green/yellow). The circuit branches into three parallel branches — each branch has a switch and an appliance (light bulb, fan, geyser). All metal bodies of appliances are connected to the earth wire via a green wire. The fuse is shown in series with the live wire before it branches. Diagram 2 (earth wire in action): A washing machine cross-section. Inside, a live wire has broken insulation and touches the metal drum. Arrow shows current path: damaged live wire → metal drum → earth wire → ground rod in earth → returns to supply. A small explosion symbol at the fuse box shows the fuse blowing. Second scenario shows what happens WITHOUT earth wire: current path through human body shown with a red bolt symbol. Diagram 3 (fuse cross-section vs MCB): Fuse: cylindrical glass tube with a thin metal strip inside. MCB: rectangular switch with an electromagnet mechanism shown inside. A comparison table below: Fuse | MCB — One-time use | Resettable — Slow | Fast — Melts | Trips — Cheaper | More expensive.",
      key_takeaway:
        "Domestic circuits use three wires: live (220 V, red/brown), neutral (0 V, black/blue), and earth (safety, green/yellow); all appliances are in PARALLEL; fuse (low-melting alloy, in series with live wire) blows on overcurrent — one-time use; MCB (electromagnetic trip, resettable) is the modern alternative; earth wire connects metal appliance bodies to ground — if live wire touches metal body, large current flows through earth wire blowing the fuse before causing electric shock; short circuit = near-zero resistance path (live + neutral touch) → huge current; overloading = excess appliances → total current exceeds fuse rating.",
    },
  },
];
