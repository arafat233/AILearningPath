...
const TOPICS = [
  T("sysd_m4_t1", 1, "Design Video Streaming (YouTube/Netflix)", "sysd-video-streaming",
    ...
    [
      ...
      { kind: "concept", heading: "Common interview follow-ups",
        body: "'Live streaming vs on-demand' (live adds low-latency ingest + near-real-time transcoding; chunked HLS/DASH with a few-second delay). 'Resumable/chunked uploads' (multipart upload to blob storage). 'Recommendations' (a separate ML pipeline over watch history). 'Global low latency' (multi-region CDN + origin shielding so the origin isn't hammered). 'View counts at scale' (approximate/streamed counters). The backbone stays: blob storage + async transcoding into ABR segments + CDN delivery." },
    ],
  ...
  T("sysd_m4_t2", 2, "Design Ride-Sharing (Uber/Lyft)", "sysd-ride-sharing",
    ...
    [
      ...
      { kind: "concept", heading: "Common interview follow-ups",
        body: "'Live driver tracking on the rider's map' (drivers stream location over WebSocket; push updates to the matched rider). 'Surge pricing' (a pricing service reading supply/demand per geo cell — a Strategy). 'ETA' (a routing/maps service). 'Matching fairness & dispatch' (offer to nearest, timeout→next; avoid double-assigning a driver with an atomic claim). 'Scale the geo index' (shard the map by region/cell). The defining challenge stays: geospatial indexing (geohash/quadtree) + latest-location in memory + fast matching." },
    ],
  ...
  T("sysd_m4_t3", 3, "Design File Sync (Dropbox/Drive)", "sysd-file-sync",
    ...
    [
      ...
      { kind: "concept", heading: "Common interview follow-ups",
        body: "'Sync a 10GB file efficiently' (content-defined chunking + dedup → upload only changed chunks). 'Real-time multi-device sync' (server notifies other devices via long-poll/WebSocket; they pull changed metadata + missing chunks). 'Sharing & permissions' (ACLs on the metadata tree). 'Bandwidth on mobile' (delta sync + compression). 'Global users' (chunks in regional blob stores + CDN). The pillars stay: chunk + dedup, the metadata/content split, and conflict copies instead of data loss." },
    ],
  ...
  T("sysd_m4_t4", 4, "Design a Web Crawler", "sysd-web-crawler",
    ...
    [
      ...
      { kind: "concept", heading: "Common interview follow-ups",
        body: "'Crawl billions of pages without re-visiting' (Bloom filter for seen URLs + URL normalization). 'Be polite' (per-domain rate limits, robots.txt, partition the frontier by host). 'Detect near-duplicate pages' (content hashing / SimHash / MinHash). 'Freshness' (re-crawl priority by change frequency). 'Avoid spider traps' (depth limits, URL-pattern blocklists). 'Distribute it' (shard by domain hash; a coordinator assigns ranges). The core stays: a priority frontier + Bloom-filter dedup + politeness." },
    ],
  ...
  T("sysd_m4_t5", 5, "Design a Distributed Key-Value Store (Dynamo)", "sysd-kv-store",
    ...
    [
      ...
      { kind: "concept", heading: "Common interview follow-ups",
        body: "'Tune consistency per request' (the R/W/N quorum dial — R+W>N for strong-ish reads). 'A node is temporarily down' (hinted handoff stores writes for it, replays on return). 'Replicas drifted' (read repair on the read path + background anti-entropy via Merkle trees). 'Range queries' (hard on a hash ring — you'd need an ordered/secondary index). 'Add capacity with no downtime' (consistent hashing moves only ~1/N keys). This IS the Dynamo paper: consistent hashing + N-replication + quorum + conflict resolution." },
    ],
  ...
  T("sysd_m4_t6", 6, "Design Ticket Booking (Ticketmaster)", "sysd-ticket-booking",
    ...
    [
      ...
      { kind: "concept", heading: "Common interview follow-ups",
        body: "'100k people, 1k seats, no overselling' (atomic conditional reserve Available→Held; a waiting room admits users in batches). 'Hold expiry' (TTL releases unpaid seats — a background sweep or Redis TTL). 'Seat-map view consistency' (held seats show as unavailable immediately; eventual is fine for the map, strong for the actual reserve). 'Payment fails after hold' (compensate: release the seat — a saga). 'Prevent bots' (rate limit + the queue). The crux stays: an atomic check-and-set on inventory with strong consistency." },
    ],
