...
const TOPICS = [
  ...
  T("sysd_m1_t3", 3, "Scaling: Vertical vs Horizontal", "sysd-scaling",
    ...
    [
      ...
      { kind: "concept", heading: "In the wild",
        body: "Real horizontal scaling: AWS Auto Scaling Groups / Kubernetes HPA add and remove instances based on CPU/QPS; '12-factor app' design keeps services stateless; sessions live in Redis or a signed JWT, not server memory. The usual scaling LADDER, cheapest first: vertical (bigger box) → add read replicas + a cache → only THEN shard. Sharding is last because it permanently complicates every query — interviewers want to see you exhaust the cheaper rungs first." },
    ],
  ...
  T("sysd_m1_t4", 4, "Load Balancing", "sysd-load-balancing",
    ...
    [
      ...
      { kind: "concept", heading: "In the wild",
        body: "AWS ALB (Application/L7 — path & host routing, TLS) vs NLB (Network/L4 — raw speed, static IPs); nginx and HAProxy as software LBs; Envoy in service meshes. Global traffic also balances at the DNS layer (Route 53 / GeoDNS, anycast) to route users to the nearest region before a regional LB takes over. So a real request often passes TWO balancing layers: DNS/geo, then an in-region L7 load balancer." },
    ],
  ...
  T("sysd_m1_t5", 5, "Caching", "sysd-caching",
    ...
    [
      ...
      { kind: "concept", heading: "Why it works — the latency numbers",
        body: "The whole point is the latency gap: an in-memory/Redis read is ~0.1–1 ms; a disk/DB query is ~5–50 ms; a cross-region call is ~100 ms. Caching the hot 10–20% of data that serves 80–90% of reads turns most requests from milliseconds into microseconds AND removes that load from the database. Quick math: at a 90% hit rate, the DB sees only 1 in 10 reads — a 10× capacity multiplier on the data tier from one component." },
      { kind: "concept", heading: "Real systems",
        body: "Redis (rich data structures, persistence, the default distributed cache) and Memcached (simple, multithreaded) for app-level caching; CDNs (CloudFront, Cloudflare, Akamai) for static/media at the edge; Varnish/nginx as reverse-proxy caches; the database's own buffer pool. Sizing rule of thumb: cache enough RAM to hold the working set (hot keys) — a too-small cache thrashes (low hit rate) and barely helps." },
    ],
  ...
  T("sysd_m1_t6", 6, "SQL vs NoSQL — Choosing a Data Store", "sysd-sql-nosql",
    ...
    [
      ...
      { kind: "concept", heading: "Specialized stores worth naming",
        body: "Beyond the big two: TIME-SERIES (InfluxDB, TimescaleDB) for metrics/IoT with time-bucketed writes; SEARCH (Elasticsearch) for full-text and faceted queries; VECTOR (Pinecone, pgvector) for embeddings/semantic search in AI apps; NewSQL (CockroachDB, Spanner) for SQL semantics WITH horizontal scale. Mentioning the right specialized store for a sub-problem (e.g. 'metrics → time-series DB, product search → Elasticsearch') signals real-world maturity." },
      { kind: "concept", heading: "A worked decision",
        body: "Design Twitter: tweets + the social graph are write-heavy at massive scale with simple access by key → wide-column (Cassandra). User accounts and billing need transactions and joins → relational (Postgres). The home-timeline cache → Redis. Full-text tweet search → Elasticsearch. That's polyglot persistence: there's no single 'the database' — you pick one per access pattern and justify each." },
    ],
  ...
  T("sysd_m1_t7", 7, "Replication & Sharding", "sysd-replication-sharding",
    ...
    [
      ...
      { kind: "concept", heading: "Real systems & how they combine the two",
        body: "Replication and sharding are usually used TOGETHER: shard for capacity, then replicate each shard for availability. Postgres/MySQL: a primary with read replicas (replication), sharded by app logic or Vitess. MongoDB: replica sets (replication) × sharded clusters. Cassandra/DynamoDB: consistent-hashing partitioning + N-way replication + quorum reads/writes, all built in. So a single key lives on N replicas of one shard." },
      { kind: "concept", heading: "The operational pain of sharding",
        body: "Sharding is powerful but costly: cross-shard JOINs and transactions become application-level work or are dropped; a query without the partition key must scatter-gather across all shards; re-sharding (splitting a hot shard) means moving live data. The lesson interviewers want: don't shard until you must — exhaust read replicas + caching + a bigger box first, because sharding permanently complicates every query and migration." },
    ],
  ...
  T("sysd_m1_t8", 8, "CAP Theorem & Consistency", "sysd-cap-consistency",
    ...
    [
      ...
      { kind: "concept", heading: "Real systems, classified",
        body: "CP (choose consistency, may reject during a partition): ZooKeeper, etcd, HBase, traditional RDBMS in a cluster, MongoDB (default). AP (choose availability, may serve stale): Cassandra, DynamoDB, Riak, CouchDB. Tunable (pick per query via quorum R+W): Cassandra and DynamoDB let you dial consistency up for a critical write and down for a cheap read. Naming where a datastore sits — and that many are tunable — is the senior signal beyond just reciting the theorem." },
      { kind: "concept", heading: "Quorum: the dial behind 'tunable'",
        body: "In a leaderless/replicated store with N replicas, a write waits for W acks and a read queries R replicas. If R + W > N the read and write sets overlap, so reads see the latest write (strong-ish). Tune it: W=N,R=1 → durable writes, fast reads, slow/fragile writes; W=1,R=1 → fast and available but stale-prone (AP). This R/W/N knob is how DynamoDB/Cassandra turn the CAP choice into a per-operation decision." },
    ],
  ...
  T("sysd_m1_t9", 9, "Message Queues & Async Processing", "sysd-message-queues",
    ...
    [
      ...
      { kind: "concept", heading: "In the wild — pick the right broker",
        body: "Kafka: a partitioned, replayable commit log — massive throughput, ordered within a partition, consumers track their offset; ideal for event streaming, logs, and fan-out to many consumer groups. RabbitMQ: a flexible work queue/broker with rich routing and acks; great for task distribution. AWS SQS: fully managed, dead-simple at-least-once queue. Redis Streams: lightweight, low-latency. Rule of thumb: streaming/replay/high fan-out → Kafka; classic task queue with routing → RabbitMQ/SQS." },
    ],
  ...
  T("sysd_m1_t10", 10, "Case Study: Design a URL Shortener", "sysd-url-shortener",
    ...
    [
      ...
      { kind: "concept", heading: "Common interview follow-ups",
        body: "'How long should the key be?' (base-62: 7 chars ≈ 3.5 trillion — do the math out loud). '301 or 302 redirect?' (302 keeps control + lets you count clicks; 301 is cached by browsers and skips your server). 'Custom/vanity URLs?' (reserve + uniqueness-check). 'Analytics at billions of clicks?' (fire events to Kafka → aggregate offline, never block the redirect). 'Expire and reclaim keys?' (TTL + a cleanup job). 'Prevent abuse?' (rate-limit creation, scan destinations). The anchor stays: it's read-dominated → cache/CDN over a KV store." },
    ],
  ...
  T("sysd_m1_t11", 11, "Case Study: Distributed Rate Limiter", "sysd-rate-limiter",
    ...
    [
      ...
      { kind: "concept", heading: "Common interview follow-ups",
        body: "'Make the counter atomic across nodes' (Redis INCR+EXPIRE or a Lua script — the crux, or two requests slip past). 'Fix the fixed-window boundary burst' (sliding-window log/counter). 'Global vs per-region limits' (a central store adds latency; per-region is faster but looser — pick per the requirement). 'Layered limits' (per-user AND per-IP AND per-endpoint, composed). 'Token bucket details' (capacity = burst allowance, refill rate = sustained rate). 'Redis down?' (fail-open for availability, usually)." },
    ],
  ...
  T("sysd_m1_t12", 12, "Case Study: Design a News Feed", "sysd-news-feed",
    ...
    [
      ...
      { kind: "concept", heading: "Common interview follow-ups",
        body: "'Where's the celebrity threshold?' (followers above ~10–100k → pull instead of push). 'Chronological or ranked feed?' (ranking adds an ML scoring service over candidate posts). 'Real-time updates?' (push new posts over WebSocket/SSE, not just on refresh). 'Pagination?' (cursor-based on post id/timestamp, never OFFSET at scale). 'How stale can the feed be?' (eventual consistency is fine — bias to availability). The discriminating insight stays: push/pull HYBRID solves the celebrity fan-out explosion." },
    ],
