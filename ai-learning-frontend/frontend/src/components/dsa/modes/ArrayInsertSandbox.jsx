/**
 * ArrayInsertSandbox — animate insert-at-index (shift-right). M30.
 *
 * D2.3 from the log2base2 parity checklist. Shows that every element
 * at index ≥ insertIdx gets bumped right by 1 — the O(n) cost of array
 * insertion. Uses the existing ArrayVisualizer so pointer labels are
 * available (L/R are named-but-fixed here, not sliding).
 */
import { useCallback, useRef, useState } from "react";
import ArrayVisualizer from "../ArrayVisualizer.jsx";
import ExplanationPanel from "../ExplanationPanel.jsx";
import HighlightedCode from "../HighlightedCode.jsx";
import { ARRAY_INSERT_CODE, LINE_BY_PHASE, generateArrayInsertSteps } from "../algorithms/arrayInsert.js";

const DEFAULT_ARR    = [10, 20, 30, 40, 50, 60];
const DEFAULT_IDX    = 2;
const DEFAULT_VALUE   = 99;

export default function ArrayInsertSandbox() {
  const [arrInput, setArrInput]     = useState(DEFAULT_ARR.join(", "));
  const [idxInput, setIdxInput]      = useState(String(DEFAULT_IDX));
  const [valInput, setValInput]      = useState(String(DEFAULT_VALUE));
  const [speed, setSpeed]           = useState(700);
  const [frames, setFrames]         = useState(() =>
    generateArrayInsertSteps(DEFAULT_ARR, DEFAULT_IDX, DEFAULT_VALUE)
  );
  const [idx, setIdx]               = useState(0);
  const [playing, setPlaying]       = useState(false);
  const stoppedRef = useRef(false);

  const frame = frames[idx] || frames[frames.length - 1];

  const rebuild = useCallback(() => {
    const arr = arrInput.split(",").map((s) => Number(s.trim())).filter((n) => Number.isFinite(n));
    const pos = Number(idxInput);
    const val = Number(valInput);
    if (arr.length === 0 || !Number.isFinite(pos) || !Number.isFinite(val)) return;
    setFrames(generateArrayInsertSteps(arr, pos, val));
    setIdx(0);
    setPlaying(false);
    stoppedRef.current = true;
  }, [arrInput, idxInput, valInput]);

  const play = useCallback(async () => {
    if (playing) return;
    setPlaying(true);
    stoppedRef.current = false;
    for (let i = idx; i < frames.length; i++) {
      if (stoppedRef.current) break;
      setIdx(i);
      // eslint-disable-next-line no-await-in-loop
      await new Promise((r) => setTimeout(r, speed));
    }
    setPlaying(false);
  }, [playing, idx, frames, speed]);

  const stop  = () => { stoppedRef.current = true; setPlaying(false); };
  const step  = () => { if (idx < frames.length - 1) setIdx(idx + 1); };
  const back  = () => { if (idx > 0) setIdx(idx - 1); };
  const reset = () => { stoppedRef.current = true; setPlaying(false); setIdx(0); };

  // Derive active line from phase — array insert has no codeLine in step
  const activeLine = LINE_BY_PHASE[frame.phase] ?? 2;

  // Build cells: highlight insertIdx and activeIdx; null slots shown as placeholder "·"
  const cells = frame.array.map((v, i) => {
    const state = i === frame.activeIdx ? "compare" : i === frame.insertIdx && frame.phase === "place" ? "found" : "default";
    return { value: v == null ? "·" : v, state };
  });

  // Named pointers: show "insertIdx" label at insertion column
  const pointers = {};
  if (frame.insertIdx >= 0 || frame.insertIdx <= frame.array.length) {
    pointers.insertIdx = frame.insertIdx;
  }

  return (
    <div className="flex flex-col gap-4 w-full bg-zinc-950 text-white p-4 rounded-2xl border border-zinc-800">
      <div className="flex flex-wrap gap-3 items-center">
        <label className="text-xs text-zinc-400">array</label>
        <input value={arrInput} onChange={(e) => setArrInput(e.target.value)} className="px-3 py-2 bg-zinc-800 border border-zinc-700 rounded-lg text-sm text-white w-56" />

        <label className="text-xs text-zinc-400">index</label>
        <input value={idxInput}  onChange={(e) => setIdxInput(e.target.value)}  className="px-3 py-2 bg-zinc-800 border border-zinc-700 rounded-lg text-sm text-white w-16" />

        <label className="text-xs text-zinc-400">value</label>
        <input value={valInput}  onChange={(e) => setValInput(e.target.value)}  className="px-3 py-2 bg-zinc-800 border border-zinc-700 rounded-lg text-sm text-white w-20" />

        <button onClick={rebuild} className="px-3 py-2 bg-zinc-700 hover:bg-zinc-600 rounded-lg text-sm text-white transition">Apply</button>

        <div className="w-px h-6 bg-zinc-700" />

        {!playing
          ? <button onClick={play}  className="px-4 py-2 bg-green-600  hover:bg-green-500  rounded-lg text-sm font-medium text-white transition">▶ Play</button>
          : <button onClick={stop}  className="px-4 py-2 bg-yellow-600 hover:bg-yellow-500 rounded-lg text-sm font-medium text-white transition">⏸ Pause</button>}
        <button onClick={back}  disabled={idx === 0}                  className="px-3 py-2 bg-zinc-700  hover:bg-zinc-600  disabled:opacity-40 rounded-lg text-sm font-medium text-white transition">◀</button>
        <button onClick={step}  disabled={idx >= frames.length - 1}   className="px-3 py-2 bg-purple-700 hover:bg-purple-600 disabled:opacity-40 rounded-lg text-sm font-medium text-white transition">Step ▶</button>
        <button onClick={reset}                                       className="px-3 py-2 bg-zinc-700  hover:bg-zinc-600  rounded-lg text-sm font-medium text-white transition">Reset</button>

        <div className="ml-auto flex items-center gap-2 text-sm">
          <span className="text-zinc-400 text-xs">Speed</span>
          <input type="range" min={150} max={1500} step={50} value={1500 - speed + 150} onChange={(e) => setSpeed(1500 - Number(e.target.value) + 150)} className="w-24 accent-blue-500" />
        </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-[1fr_440px] gap-4">
        <ArrayVisualizer cells={cells} pointers={frame.insertIdx >= 0 && frame.insertIdx <= frame.array.length ? { "insertIdx →": frame.insertIdx } : {}} />
        <HighlightedCode code={ARRAY_INSERT_CODE} activeLine={activeLine} height="280px" />
      </div>

      <ExplanationPanel text={`${frame.step.description}${frame.step.detail ? " — " + frame.step.detail : ""}`} />
      <div className="text-[11px] text-zinc-500 font-mono">frame {idx + 1} / {frames.length}</div>
    </div>
  );
}
