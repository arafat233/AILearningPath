/**
 * DutchFlagSandbox — 3-way partition with low/mid/high pointers. M30-T4.
 */
...
export default function DutchFlagSandbox() {
  const [arrInput, setArrInput] = useState(DEFAULT_ARR.join(", "));
  const [speed, setSpeed]       = useState(700);
  const [frames, setFrames]     = useState(() => generateDutchFlagSteps(DEFAULT_ARR));
  ...
  const rebuild = useCallback(() => {
    const arr = arrInput.split(",").map((s) => Number(s.trim())).filter((n) => [0, 1, 2].includes(n));
    if (arr.length === 0) return;
    setFrames(generateDutchFlagSteps(arr));
    setIdx(0);
    setPlaying(false);
  ...
  // Translate the algorithm's region states into ArrayVisualizer cell states.
  const cells = frame.array.map((v, i) => ({ value: v, state: frame.states[i] || "default" }));

  return (
    <div className="flex flex-col gap-4 w-full bg-zinc-950 text-white p-4 rounded-2xl border border-zinc-800">
      <div className="flex flex-wrap gap-3 items-center">
        <label className="text-xs text-zinc-400">array (use 0, 1, 2)</label>
