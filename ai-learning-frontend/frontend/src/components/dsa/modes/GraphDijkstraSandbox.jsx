/**
 * GraphDijkstraSandbox — Dijkstra's single-source shortest paths. M37-T4.
 */
...
export default function GraphDijkstraSandbox() {
  ...
  const [frames, setFrames] = useState(() => generateDijkstraSteps(DIJKSTRA_GRAPH, "A"));
  ...
  const rebuild = useCallback(() => {
    setFrames(generateDijkstraSteps(DIJKSTRA_GRAPH, src));
    setIdx(0);
    setPlaying(false);
    stoppedRef.current = true;
  }, [src]);

  useEffect(() => { rebuild(); /* eslint-disable-next-line react-hooks/exhaustive-deps */ }, [src]);
  ...
  return (
    <div className="flex flex-col gap-4 w-full bg-zinc-950 text-white p-4 rounded-2xl border border-zinc-800">
      <div className="flex flex-wrap gap-3 items-center">
        ...
        <select value={src} onChange={(e) => setSrc(e.target.value)} className="px-3 py-2 bg-zinc-800 border border-zinc-700 rounded-lg text-sm text-white">
          {DIJKSTRA_GRAPH.nodes.map((n) => <option key={n.id} value={n.id}>{n.id}</option>)}
        </select>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <div className="rounded-xl border border-zinc-800 bg-zinc-900 px-4 py-3">
          <div className="text-[10px] uppercase tracking-widest text-[#555] mb-1.5">priority queue</div>
          <div className="font-mono text-[13px] text-blue-300">{frame.pqSnapshot.length === 0 ? <span className="text-[#555] italic">empty</span> : `[${frame.pqSnapshot.join(", ")}]`}</div>
        </div>
        <div className="rounded-xl border border-zinc-800 bg-zinc-900 px-4 py-3">
          <div className="text-[10px] uppercase tracking-widest text-[#555] mb-1.5">visited (finalised)</div>
          <div className="font-mono text-[13px] text-emerald-300">{frame.visited.length === 0 ? <span className="text-[#555] italic">none</span> : `[${frame.visited.join(", ")}]`}</div>
        </div>
      </div>

      <ExplanationPanel text={`${frame.step.description}${frame.step.detail ? " — " + frame.step.detail : ""}`} />
      <div className="text-[11px] text-zinc-500 font-mono">frame {idx + 1} / {frames.length} · numbers below nodes = current tentative distance</div>
    </div>
  );
}
