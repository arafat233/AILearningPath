/**
 * LRUSandbox — LRU cache demo with paired HashMap + DLL view. M34-T5.
 */
...
function parseOps(text) {
  ...
  for (const line of text.split("\n")) {
    ...
    if (parts[0] === "put" && parts.length === 3) {
      const k = isNaN(Number(parts[1])) ? parts[1] : Number(parts[1]);
      const v = isNaN(Number(parts[2])) ? parts[2] : Number(parts[2]);
...
export default function LRUSandbox() {
  const [capInput, setCapInput] = useState("3");
  const [opsText, setOpsText]   = useState(DEMO_OPS.map((o) => o.type === "put" ? `put ${o.key} ${o.value}` : `get ${o.key}`).join("\n"));
  const [speed, setSpeed]   = useState(800);
  const [frames, setFrames] = useState(() => generateLRUSteps(3, DEMO_OPS));
  ...
  const rebuild = useCallback(() => {
    const cap = Math.max(1, Math.floor(Number(capInput) || 1));
    const ops = parseOps(opsText);
    setFrames(generateLRUSteps(cap, ops));
    setIdx(0);
    setPlaying(false);
  ...
  return (
    <div className="flex flex-col gap-4 w-full bg-zinc-950 text-white p-4 rounded-2xl border border-zinc-800">
      <div className="flex flex-wrap gap-3 items-center">
        ...
        <input value={capInput} onChange={(e) => setCapInput(e.target.value)} className="px-3 py-2 bg-zinc-800 border border-zinc-700 rounded-lg text-sm text-white w-16" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-4">
        <LRUVisualizer capacity={frame.capacity} order={frame.order} mapKeys={frame.mapKeys} evicted={frame.evicted} />
        <div className="space-y-3">
          ...
          <div className="rounded-xl border border-zinc-800 bg-zinc-900 px-4 py-3">
            <div className="text-[10px] uppercase tracking-widest text-[#555] mb-1.5">last op</div>
            <div className="font-mono text-[13px] text-amber-300">{frame.lastOp || <span className="text-[#555] italic">—</span>}</div>
            {frame.hit === true && <div className="mt-1 text-[11px] text-emerald-400">HIT</div>}
            {frame.hit === false && <div className="mt-1 text-[11px] text-red-400">MISS</div>}
          </div>
        </div>
      </div>
