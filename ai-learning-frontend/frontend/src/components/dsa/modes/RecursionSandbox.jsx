/**
 * RecursionSandbox — call-stack-frame animation for classic recursion.
 * M2-T5 wire-up. D1.2 from the log2base2 parity checklist.
 */
import { useCallback, useEffect, useRef, useState } from "react";
import CallStackVisualizer from "../CallStackVisualizer.jsx";
import HighlightedCode from "../HighlightedCode.jsx";
import ExplanationPanel from "../ExplanationPanel.jsx";
import { DEMO_IDS, DEMO_LABELS, DEMO_DEFAULTS, generateRecursionSteps } from "../algorithms/recursion.js";

export default function RecursionSandbox() {
  const [demoId, setDemoId] = useState(DEMO_IDS[0]);
  const [nInput, setNInput] = useState(String(DEMO_DEFAULTS[DEMO_IDS[0]]));
  const [speed, setSpeed]   = useState(700);
  const [frames, setFrames] = useState(() => generateRecursionSteps(DEMO_IDS[0], DEMO_DEFAULTS[DEMO_IDS[0]]));
  ...
  // Reset frames when demo changes
  useEffect(() => {
    const defaultN = DEMO_DEFAULTS[demoId];
    setNInput(String(defaultN));
    setFrames(generateRecursionSteps(demoId, defaultN));
    setIdx(0);
    setPlaying(false);
  ...
  const rebuild = useCallback(() => {
    setFrames(generateRecursionSteps(demoId, Number(nInput)));
    setIdx(0);
    setPlaying(false);
    stoppedRef.current = true;
  }, [demoId, nInput]);
  ...
  return (
    <div className="flex flex-col gap-4 w-full bg-zinc-950 text-white p-4 rounded-2xl border border-zinc-800">
      <div className="flex flex-wrap gap-3 items-center">
        ...
        <input value={nInput} onChange={(e) => setNInput(e.target.value)} className="px-3 py-2 bg-zinc-800 border border-zinc-700 rounded-lg text-sm text-white w-16" />
        ...
        <div className="ml-auto flex items-center gap-2 text-sm">
          ...
          <input type="range" min={200} max={1800} step={50} value={1800 - speed + 200} onChange={(e) => setSpeed(1800 - Number(e.target.value) + 200)} className="w-24 accent-blue-500" />
        </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-[1fr_440px] gap-4">
        <CallStackVisualizer frames={frame.callStack} maxFrames={10} />
      ...
      <div className="text-[11px] text-zinc-500 font-mono">frame {idx + 1} / {frames.length} · stack depth peaks at {Math.max(...frames.map((f) => f.callStack.length))}</div>
    </div>
  );
}
