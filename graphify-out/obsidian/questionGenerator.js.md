---
source_file: "E:\AILearningPath\ai-learning-backend\backend\utils\questionGenerator.js"
type: "code"
community: "Community 12"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_12
---

# questionGenerator.js

## Connections
- [[ap()]] - `contains` [EXTRACTED]
- [[appsTrig()]] - `contains` [EXTRACTED]
- [[areasCircles()]] - `contains` [EXTRACTED]
- [[circles()]] - `contains` [EXTRACTED]
- [[coordinateGeometry()]] - `contains` [EXTRACTED]
- [[diff()]] - `contains` [EXTRACTED]
- [[ds()]] - `contains` [EXTRACTED]
- [[gcd()]] - `contains` [EXTRACTED]
- [[linear()]] - `contains` [EXTRACTED]
- [[polynomials()]] - `contains` [EXTRACTED]
- [[probability()]] - `contains` [EXTRACTED]
- [[quadratic()]] - `contains` [EXTRACTED]
- [[rand()]] - `contains` [EXTRACTED]
- [[realNumbers()]] - `contains` [EXTRACTED]
- [[save()]] - `contains` [EXTRACTED]
- [[statistics()]] - `contains` [EXTRACTED]
- [[surfaceVolumes()]] - `contains` [EXTRACTED]
- [[triangles()]] - `contains` [EXTRACTED]
- [[trigonometry()]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_12